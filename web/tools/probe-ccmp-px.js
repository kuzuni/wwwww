...
const MEASURE = (async (srcs) => {
    ...
    for (const src of srcs) {
        ...
        // 🚨 무채색 허용 10→16 (2026-08-19): 클론 카드 바탕이 AAA 스킨에서 **따뜻한 크림색**으로
        //    바뀌었다 — 카드 좌우 흰 여백 실측이 원본 `255,255,255`(채도 0) vs 클론
        //    `250,246,240`(채도 10) · `246,242,235`(채도 **11**)이다. 10 문턱은 이 크림을 **줄마다
        //    들쭉날쭉 떨궈** 흰 성분의 세로 연결이 y662 에서 끊겼고(원본은 y776 까지 이어진다),
        //    그 바람에 `yBtn` 밴드가 버튼 대신 새 장비 타일을 물어 **'버튼을 못 잡음'으로 3/3 고장**났다.
        //    이 프로브는 **기하만** 판정하므로(색 이탈은 AAA 스킨의 의도된 변경 — 리그 밴드·채팅 바와
        //    같은 판례) 크림을 흰 것으로 받아 준다. 회색 패널(#bebebe 190)은 채도가 아니라 **밝기
        //    215 문턱**으로 걸러지므로 이 완화로 패널이 흰 성분에 섞이지는 않는다.
        const white = (x, y) => { const p = at(x, y); return p[0] >= 215 && p[1] >= 215 && p[2] >= 215 && (Math.max(...p) - Math.min(...p)) <= 16; };
