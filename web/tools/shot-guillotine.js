...
(async () => {
    ...
    await page.evaluate(() => {
        ...
        Scene3D.skillEffect('guillotine', 0xe57373, ids, { rarity: 'mythic' });
    });
    ...
    for (let i = 0; i < stamps.length; i++) {
        ...
        await page.screenshot({ path: path.join(__dirname, `guillotine-${i}.png`) });
    }
    console.log('캡처 완료: guillotine-0..' + (stamps.length - 1) + '.png (t=' + stamps.join(',') + 's)');
    await browser.close();
})();
