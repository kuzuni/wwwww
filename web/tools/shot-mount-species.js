...
(async () => {
    ...
    const n = await page.evaluate((argNames) => {
        ...
        const cells = names.map(nm => {
            ...
            return `<div style="width:${w * 2 + 6}px;margin:3px;background:#20242b;border-radius:10px;padding:4px">
                ...
                <div style="font:11px sans-serif;color:#9fb0c0;text-align:center">${MOUNT_KR[nm] || nm} / ${nm}</div></div>`;
        }).join('');
    ...
    await page.locator('#sheet').screenshot({ path: path.join(__dirname, 'mount-species.png') });
    await browser.close();
    console.log(`→ tools/mount-species.png · ${n}종 · 콘솔 에러 ${errors.length}건`);
