...
assertFresh('tools/ref-cmp/clone/league-rewards.png', ['web/js', 'web/css'], 'node tools/shot-lc.js  # 또는 shot-screens.js', { warnOnly: true });
...
const MEASURE = async ([src]) => {
    ...
    /* 🚨 가로를 좁히는 것만으로는 부족했다 — **행 밀도 게이트가 하나 더 필요하다**
         (2026-08-20 UI, red-probes-outside-regress). 리본이 이미 같은 처방을 쓰고 있는데(위 `dense`)
         ✕ 만 빠져 있었다.
       왜 이제야 드러났나: 이 자의 ✕ 스캔 창은 **표 아래끝(ty1)에서 시작**한다. 카드가 흰 종이로
       덮여 있던 동안에는 ty1 이 744 로 잘못 잡혀 창이 y726 부터라, 그 위의 오염을 **우연히**
       비켜 갔다. 카드를 원본대로 되돌리자 ty1 이 705(정상)로 내려오면서 창이 y687 로 넓어졌고,
       거기서 **표 마지막(세로로 잘린) 행의 아이콘 화소**가 잡혔다.
       실측 오염: y708~721 에 **행당 1~2 화소**, 색은 `rgb(159,84,8)` 계열 = 짙은 주황이다.
       `isRed`(r>150 · g<90 · b<90)의 g<90 을 **84 로 아슬아슬하게** 통과한다 — 빨강이 아니라
       주황인데도 술어를 뚫는다. 그 결과 ✕ 상단이 748 → 708 로 끌려 올라가 **높이가 두 배**
       (5.27 → 9.75%H)로 읽혔다. ✕ 자체는 멀쩡했다(`probe-lgr-dom` 이 DOM 으로 재서 지름·상단
       Δ+1.28/−0.07%p 로 통과, 원본 확대와 모양도 같다).
       처방: **한 행에서 빨강이 6 화소 이상**일 때만 그 행을 ✕ 로 친다. ✕ 는 지름 ~50px 원이라
       맨 윗줄에서도 13 화소가 나오고(원본 y746 n=13 · 클론 y748 n=8), 오염은 1~2 화소다 —
       사이가 4배 이상 벌어져 있어 임계가 예민하지 않다. **원본·클론에 같은 코드가 걸린다**
       (채점 함정 ①: 한쪽만 다른 기준으로 재면 안 된다).
       ⚠️ 색 술어(`isRed`)를 조이는 쪽으로 고치지 말 것 — ✕ 본체에도 `rgb(155,16,18)` 같은
          어두운 가장자리가 있어서 g 임계를 내리면 이번엔 ✕ 가 깎인다(실측). 색이 아니라
          **덩어리 크기**가 둘을 가르는 축이다. */
    const RED_MIN_PER_ROW = 6;
    const bboxX = (test, y0, y1) => {
        ...
        for (let y = y0; y < y1; y++) {
            ...
            for (let x = xLo; x <= xHi; x++) {
                if (!test(at(x, y))) continue;
                n++; if (x < rx0) rx0 = x; if (x > rx1) rx1 = x;
            }
            if (n < RED_MIN_PER_ROW) continue;          // 잘린 표 행의 주황 아이콘 부스러기를 버린다
