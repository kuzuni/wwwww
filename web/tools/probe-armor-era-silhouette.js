...
// 🚨 게이트 표 리셋 (2026-08-21, equip-build-armor): 이 게이트의 모항목 `equip-era-theming` 은
//    사용자 지시로 **취소**됐고(오케스트레이션 담당 → 루틴 스코프 제외), 갑옷은 승인된 새 스펙
//    "마인크래프트 박스 캐릭터 시대 테마 복셀"(채점 불필요)로 전량 교체됐다. 옛 곡면 조형이
//    세션 단위로 갈라 두었던 96px 실루엣 분화 축은 새 파라메트릭 의상 체계에서 성립하지 않는다 —
//    시대 판독은 팔레트·시대 부속(어깨판/코트 자락/등팩/가시/깃 등)이 진다. 이 프로브는 측정
//    전용으로 남긴다(아래 표를 다시 채우면 그 칸부터 게이트 부활).
const GATED = {
