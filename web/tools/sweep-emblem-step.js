...
//         로 ⓑ(검정 뺀 채움 채도)를 재서 실제 화면에서 얼마나 움직였는지 확인할 것.
//         실측(2026-08-20 UI 스트림): `.30 → .24` 한 칸에서 이 열은 51.4 → 56.8 (+5.4%p),
//         화면에서 잰 ⓑ 는 45.6 → 50.0 (+4.4%p). **방향과 크기가 대체로 맞는다.**
//      🚨 **그 확인을 할 때 `probe-skill-orb-ink` 는 반드시 클론 캡처를 다시 구운 뒤에 읽을 것.**
//         그 자는 브라우저를 안 띄우고 **커밋된 PNG 두 장을 맞대기만 한다** — 캡처가 낡으면
//         옛 화면의 수치를 아무 경고 없이 인쇄한다. 이 세션이 실제로 거기 걸려
//         "`.30 → .24` 는 ⓑ 무변 = 이 노브는 죽은 레버"라는 **틀린 결론**을 한 번 냈다(커밋 하나가
//         그 상태로 나갔고 바로 다음 커밋에서 정정했다). 지금은 `clone-fresh.js` 가 그 자를
//         낡은 캡처 위에서 못 돌게 끊는다. 사고 전말은 그 파일 머리말에 있다.
//
