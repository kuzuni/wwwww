...
//   시즌 종료 바  y 145~167 (16.18%H · h 2.57%H) · x 144~354 (w 42.17%W)
//     🚨 **여기 적혀 있던 `x 126~354 (45.98%W)` 는 뒤에 픽셀 교차검증으로 뒤집힌 값이다**
//        (`css/style.css .league-season-bar` 주석의 '함정 ①'): 126 은 **바 왼쪽 바깥에 걸친 선물
//        아이콘**(x126~152)까지 한 덩어리로 잰 것이고, 바 자체는 x144~354 · 210px 이다.
//        CSS 는 그 교차검증 값(42.17%W)으로 이미 고쳐졌는데 **이 자만 옛 목표에 머물러**,
//        사양대로 그린 화면을 계속 FAIL 로 찍고 있었다(red-probes-league ⑴).
//        이 자는 `.league-season-bar` **요소 rect** 를 재므로(절대배치 아이콘은 밖이라 안 들어온다)
//        목표도 바 기준이어야 앞뒤가 맞는다.
