...
//
// 🚨 **`grayFoot` 값(y 70.2%H)은 믿지 말 것 — 오염됐다.** 회색 마스크가 랭킹 행 안의 밝은 아바타까지
//    잡아 밴드 상단을 38px 위로 읽는다. 실제 밴드 상단은 **74.44%H(y=667)** 다 — 왼쪽 가장자리
//    x=6 열의 세로 색 변화로 확인했다(665~666 검정 테두리 → 667 부터 rgb(175,175,175), 819 에서 끝).
//    이 값을 좇아 레이아웃을 고치면 밴드가 2.1%p 올라간다(2026-08-18 UI 세션에서 실제로 한 번 밟았다).
//    밴드·[도전] 버튼 회귀는 `probe-league-header.js` 로 볼 것.
