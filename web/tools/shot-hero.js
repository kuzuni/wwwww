...
(async () => {
    ...
    // 🚨 **90000 → 180000 (2026-08-20, voxel 전환 중 실측으로 원인 확정).** 90s 는 이미 아슬아슬한
    //    값이었고, 영웅 리그가 voxel 로 바뀌며 삼각형이 21.8k → 50.8k 로 늘자 **매번** 넘겼다.
    //    ⚠️ 원인을 '무거워졌다'로 읽고 조형을 되돌리지 말 것 — `probe-hero-frametime` 으로 A/B 를
    //    쟀더니 프레임 중앙값은 168.3/167.3/168.3ms → 182.1/180.6/184.5ms, **+8%** 뿐이다
    //    (삼각형이 2.3배인데 8%인 이유: 이 씬은 정점이 아니라 필레이트·나머지 작업이 지배한다).
    //    즉 8% 가 90s 문턱을 넘긴 것이지 느려서 못 쓸 상태가 된 게 아니다. **고정 대기 문턱이
    //    실제 소요의 코앞에 있으면 아무 변경이나 이걸 터뜨린다** — 이 저장소 함정 ⑤ 의 같은 얼굴이다.
    await page.waitForFunction(() => window.__frozen === true, null, { timeout: 180000, polling: 30 }); // 슬로모 0.12× — 돌진 4×+스윙 4.2s 후 0.64 위상 도달까지 여유
    await page.evaluate(() => {
