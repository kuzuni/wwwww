...
// ── ⑧ 🚨 '속 파내기'가 함정이라는 사실을 잠근다 ──────────────────────────────
// 착수 때 `Voxel.hollow`(사방이 막힌 칸 버리기)를 넣었다가 이 시험에서 뒤집혔다.
//   기대는 "칸만 줄고 면은 그대로"였는데, 실제로는 **면이 는다** — 파낸 공동의 안쪽 벽이
//   드러나기 때문이다. 면 제거 규칙 ⓑ 덕에 속 칸의 렌더 비용은 애초에 0 이었다.
//   그래서 헬퍼를 지웠고, 대신 그 산수를 여기 남긴다(다음 세션이 다시 넣지 않도록).
{
    const solid = V.box(6, 6, 6);
    eq('6³ 속 찬 덩어리의 겉면', V.faces(solid, {}).length, 216);
    // 손으로 속을 판 것(= 지운 헬퍼가 하던 일)과 대조.
    ...
    ok('그래서 Voxel.hollow 는 존재하지 않는다', typeof V.hollow === 'undefined');
}
