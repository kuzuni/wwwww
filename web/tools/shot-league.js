// 리그 화면 한 장만 캡처 (원본 shot-042149 대조용) — 전체 31화면 캡처보다 빠른 반복용
// 사용: PW_PATH=<playwright> node shot-league.js [출력png]
//
// shot-screens.js·shot-skills.js 의 캡처 규약을 그대로 따른다(안 지키면 캡처가 오염된다):
//   · 시드를 넣고 **reload** 해야 시드 상태로 부팅한 화면이 나온다
//   · `UI.toast` 를 막는다 — 전투 틱이 캡처 직전에 '첫 클리어' 토스트를 띄워 목록을 덮은 실제 사례
//   · 자동 제련이 캡처 대기 중 제작을 돌려 **비교 팝업이 화면을 통째로 덮는다** — showCraftModal 를 막는다
//   · 카드 열림 애니(`cardpop` scale .7)가 남아 있으면 문장·행 치수가 0.7배로 찍힌다 → `.opening` 제거
//   · `document.fonts.ready` 를 상한을 걸어 먼저 소화하지 않으면 screenshot 이 폰트 대기에서 타임아웃 난다
const { chromium } = require(process.env.PW_PATH || '/opt/node22/lib/node_modules/playwright');
const path = require('path');
const INDEX = 'file://' + path.resolve(__dirname, '../index.html');
const { waitReady } = require('./wait-ready.js');
const { SEED_SRC } = require('./shot-screens-seed.js');
const OUT = process.argv[2] || path.join(__dirname, 'ref-cmp/clone/league.png');
