...
// 사용: node shot-style-eras.js <스타일> [출력경로] [슬롯]   예: node shot-style-eras.js cape
//       슬롯 기본 armor · 'helmet' 을 주면 HELMET_STYLES 를 찾는다 (예: node shot-style-eras.js crown '' helmet)
...
const SLOT = (process.argv[4] || 'armor');
...
(async () => {
    ...
    const info = await page.evaluate(({ STYLE, SLOT }) => {
        ...
        const CELLS = AGES.map(age => {
            const i = ((SLOT === 'helmet' ? HELMET_STYLES : ARMOR_STYLES)[age] || []).indexOf(STYLE);
        ...
        const html = CELLS.map(([age, i]) => {
            ...
            const t = Scene3D.itemThumb({ slot: SLOT, age, ageIdx: ai, rarity: 'common', level: 1, main: 'hp', value: 1, subs: [], nameIdx: i });
            const v = variantOf(age);
            return `<div style="width:288px;text-align:center"><div style="${cellBg(age)}width:282px;height:282px;display:flex;align-items:center;justify-content:center"><img src="${t}" style="width:268px;height:268px;${OUTF}"></div><div style="font:13px sans-serif;color:#111">${AGE_KR[age]} · ${itemNameOf({ slot: SLOT, age, nameIdx: i })}${v ? ' <b>[' + v + ']</b>' : ''}</div></div>`;
        }).join('');
