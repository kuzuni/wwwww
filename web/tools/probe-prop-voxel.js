// 배경 식생 프롭이 **정말로 큐브 조형인지** 기계적으로 재는 게이트 (`cute-art-direction` ③ 배경).
//
...
const PROPS = [
    ['makePine', [1], [1.5, 2.0], [0.9, 1.35]],
    ['makePine', [1, true], [1.5, 2.0], [0.9, 1.35]],
...
(async () => {
    ...
    let fail = 0;
    console.log('프롭            메시  비축정렬법선  색없음  중복면   높이(×s)        폭(×s)');
    for (const r of rows) {
        ...
        if (!(okAxis && okColor && okDup && okH && okW)) fail++;
        console.log(
    ...
    console.log(fail === 0 && errs.length === 0 ? '판정: PASS — 식생 프롭 전부 축정렬 큐브 조형' : `판정: FAIL (${fail}종 미달)`);
    await browser.close();
})();
