...
(async () => {
    ...
    const out = await page.evaluate((SIDE) => {
        ...
        // 이게 전후 비교에 안전한 이유: 반지름만 바꾸는 수정은 스커트 밑단·고젯의 **높이를 안 건드리므로**
        // yBot/yTop 행이 고정돼 같은 분율이 같은 해부 위치를 가리킨다.
        // ⚠️ 단 **몸통의 위아래 끝을 늘리는 수정은 이 전제를 깬다.** 2026-08-18 에 스탠딩 고젯 칼라를
        //    넣어 yTop 이 고젯(0.502)에서 칼라 상단(0.547)으로 올라갔고, 그 바람에 '어깨선 비수평도'가
        //    0.44 → 0.55 로 나빠진 것처럼 찍혔다 — 어깨가 나빠진 게 아니라 분율 0.86~0.97 구간이
        //    이제 **칼라(위로 좁아지는 파츠)** 를 가리키게 된 것이다. 몸통 상·하단을 바꾼 커밋 전후로는
        //    이 지표를 비교하지 말고, 비교하려면 같은 코드에서 칼라를 끄고 재라.
