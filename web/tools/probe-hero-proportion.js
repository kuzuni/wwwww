// 영웅 비례 실측 — 6차 비평가 ㉢(양쪽 지목, 이 항목 최대 결함) 판정기.
//   지적: "두신비가 짧고 다리가 짧다. A 5.0두신·다리 37% / B 4.2두신·다리 26~31%.
//          원신 성인은 7~7.5두신·다리 50%+."
//   처방: 투구 0.74배 축소 + 대퇴·정강이 합쳐 +40% 연장(상반신 유지), 가랑이를 전신 중점으로.
//
...
// 판정: 두신비 ≥ 6.60 · 다리비 ≥ 0.490 · 전신높이 1.785 ±3% · 접지 -0.053 ±0.01 · 콘솔 에러 0.
//   두신비 하한을 7.0 이 아니라 6.60 으로 둔 이유: 머리를 더 줄이면 양쪽 비평가가 '되돌리지 말 것'
//   으로 꼽은 원거리 가독성·투구 정면성 신호가 먼저 무너진다. 6.79(현재)가 그 둘을 지키면서
//   낼 수 있는 상한이고, 남은 0.2~0.7두신은 상반신(목·흉곽) 연장 없이는 못 메운다 — TODO 잔여로 남겼다.
...
(async () => {
    ...
    console.log('두신비           ' + f(out.heads, 2) + '  (목표 ≥ 6.60 · 원신 7~7.5)');
    console.log('다리비           ' + f(out.legRatio * 100, 1) + '%  (목표 ≥ 49.0% · 원신 50%+)');
    ...
    const okHeads = out.heads >= 6.60;
    const okLeg = out.legRatio >= 0.490;
