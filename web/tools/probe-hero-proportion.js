...
//   사용자 원문 "주인공 캐릭터 쨋든 치비 형으로 해줘" — 치비 = 큰 머리·짧은 다리.
//   🔁 **사용자 추가 2026-08-19 "머리가 좀 더 컸으면 좋겠음"** — 머리 배율 1.0 → 1.30 으로 한 단계
//      더 키웠다(두신비 2.93 → 2.48). 그래서 이 게이트의 대역도 **2.30~3.20 → 2.15~2.70** 으로
//      내렸다. 상한 2.70 이 종전 2.93 을 밖으로 밀어내므로 머리를 다시 줄이면 여기서 걸린다.
...
// 판정: 두신비 2.15~2.70(치비 대역 — 머리 더 크게 반영) · 다리비 ≤ 0.360(짧은 다리) · 전신높이 1.785 ±3% ·
...
(async () => {
    ...
    console.log('두신비           ' + f(out.heads, 2) + '  (목표 2.15~2.70 · 머리 1.30배 치비)');
    ...
    const okHeads = out.heads >= 2.15 && out.heads <= 2.70;
