// slash 공격 연속 프레임 (예비동작→스윙→팔로스루) — 1930s 카툰 모션(스쿼시&스트레치·까딱임)을 눈으로 보기 위한 캡처.
...
(async () => {
    ...
    await page.evaluate(() => {
        ...
        window.__poseAt = (u) => { R.play('slash', true); R._t = R._clip.dur * u; R.update(0); };
    });
    ...
    for (let i = 0; i < N; i++) {
        ...
        const name = `slash-seq-${String(Math.round(u * 100)).padStart(3, '0')}.png`;
