...
(async () => {
    ...
    await page.evaluate(() => {
        ...
        Scene3D.skillEffect('nova', 0xffb74d, ids, { rarity: 'mythic' });
    });
    ...
    for (let i = 0; i < stamps.length; i++) {
        ...
        await page.screenshot({ path: path.join(__dirname, `nova-${i}.png`) });
    }
    console.log('캡처 완료: nova-0..' + (stamps.length - 1) + '.png (t=' + stamps.join(',') + 's)');
    await browser.close();
})();
