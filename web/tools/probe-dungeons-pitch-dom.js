 ...
 *   · 2026-08-25 블록화가 배너 하늘을 순백으로 밈 → 배너 속살이 **흰 여백과 같은 술어**에 걸려
 *     밴드가 17~34px 지각 → 피치 122 → 140px = **+2.04%p 거짓 FAIL**(두 세션이 '선재 결함'으로 등재)
 * 두 번 다 **CSS 는 한 줄도 안 바뀌었다.** 즉 그 축의 빨강·초록은 레이아웃이 아니라 그림이
 * 정하고 있었다.
 * 📌 그쪽 술어는 `b9a9974` 에서 '어두운 키라인이 카드 폭만큼 뻗는가'로 갈아 끼워져 지금은 배너
 *    아트에 안 흔들린다. 그래도 이 자를 따로 두는 이유: **술어는 또 바뀔 수 있고**, 그 커밋은
 *    메시지에도 TODO 에도 그 사실을 안 남겨 세션 둘이 유령을 쫓았다. DOM 축은 술어가 없다. 이 자는 **아트와 무관한 답**을 준다 — `getBoundingClientRect` 로 카드 위치를
