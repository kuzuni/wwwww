...
const T = [
    // key            라벨                    x     w     y     h
    // statBar 폭/좌단은 **알약 세로 중앙 행(y62)** 실측이다: x80~412 = w333. 옛 값 87/319 는 끝이
    // 둥근 알약을 상/하단 행 근처에서 잰 코너 실측이라 14px 좁았고, CSS 도 같은 값을 쓰고 있어서
    // 이 게이트는 초록인데 비평가 2인이 폭 −2.88%p 를 교집합으로 지적했다(bonus-band-width).
    // DOM 상자는 코너 라운딩과 무관하게 알약 전폭이므로 중앙 행 실측과 대조해야 맞다.
    ['statBar',   '스탯 요약바',              80,  333,   53,   19],
    ...
    // 🚨 w169(34.07%W) 는 오측정 화석 — 042340 실측 x177~315(키라인 포함) = **139px**(28.02%W).
    //    펫 042356/042445 의 독립 실측(139px=28.37%W)과 픽셀 단위로 일치한다(채점 1라운드 정정).
    ['summonBtn', '소환 버튼',               177,  139,  653,   68],
