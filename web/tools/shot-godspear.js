...
(async () => {
    ...
    await page.evaluate(() => {
        ...
        Scene3D.skillEffect('spear', 0xffd54f, ids, { rarity: 'mythic' });
    });
    ...
    for (let i = 0; i < stamps.length; i++) {
        ...
        await page.screenshot({ path: path.join(__dirname, `godspear-${i}.png`) });
    }
    console.log('캡처 완료: godspear-0..' + (stamps.length - 1) + '.png (t=' + stamps.join(',') + 's)');
    await browser.close();
})();
