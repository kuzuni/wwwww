...
//
// 🚨 **`delta` 의 두 항은 안정성이 다르다 — 이 숫자 하나로 세션 간 비교를 하지 말 것.**
//    `heroMean` 은 영웅 화소에서만 나오므로 재현성이 높다(같은 코드로 4런: 103.3 / 104 / 87.9 …
//    각 코드 상태 안에서는 ±1 안). 반면 `bgMean` 은 **영웅 뒤에 무엇이 있었느냐**라, 바이옴이
//    스크롤되고 시간대가 도는 이 씬에서는 런마다 달라진다 — 코드를 안 바꾸고도 108.4 ~ 121 을 봤다
//    (실측, 2026-08-19 3D 세션). 즉 게이트 45 에는 **±6 정도의 배경 노이즈가 얹혀 있다.**
//    → 개선을 판정할 때는 `heroMean` 을 먼저 볼 것(비평가 처방 = 판금 베이스 **L 70~85**).
//      델타는 같은 런 안에서의 상대 비교로 쓰고, 경계값(43~47)이 나오면 두세 번 재서 판단할 것.
//
// 부위별로 '어디가 밝은지'를 봐야 하면 `probe-hero-value-parts.js` 를 쓸 것 — 이 도구는 캐릭터
// 전체 한 숫자만 준다. 여러 세션이 그 한 숫자만 보고 판금 albedo 를 반복해 내렸는데, 실제 범인은
// 치비 전환 뒤 화면 면적이 커진 **투구(L 141)와 검(L 170)** 이었다.
//
