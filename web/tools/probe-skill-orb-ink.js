...
//
// 🚨 **그 '닫혀 있다'는 오브 면이 평평할 때만 성립한다 — 2026-08-20 이전엔 안 평평했고, 지금은 평평하다.**
//   ✅ **해결됨 (2026-08-20 UI 스트림 `orb-face-vs-ref`, 커밋 하나).** 아래는 그 전말이다 —
//      **이 자의 수치를 읽기 전에 한 번은 읽을 것.** 세 세션이 여기서 헛수치를 좇았다.
//
//   ▶ **무엇이 틀렸었나**: 판정은 '오브 바탕 = 휘도 최빈값' 하나로 두고 **거기서 40 이상 벌어진 건
//     전부 모티프'라고 친다.** 원본 오브 면은 납작해서 그 전제가 맞는데, 우리 오브는 **등급색
//     그라디언트 + 좌상단 스펙큘러 + 바깥 글로우**라 **면 자체가 잉크로 잡혔다.**
//     결정적 대조였다 — 글리프를 **하나도 안 그리고** 재도 잉크율 **16.5%** · ⓑ **18.6%** 가 나왔다.
//     그래서 '원본 34.8% ↔ 클론 55.1%'(+20.3%p)를 **글리프 탓으로 읽은 세 세션이 전부 틀렸다.**
//     실제로 글리프 쪽 노브 5개를 극단으로 흔들어도 ⓑ 는 ±2.6%p 안쪽으로만 움직였다
//     (팔레트 채도 .692→1.0 +2.6 · 글로우 끔 +2.3 · 키라인 0 −2.2 · `floorL` 116→0 0.0 · 접지 그림자 끔 0.0).
//
//   ▶ **어떻게 고쳤나**: 자를 고친 게 아니라 **화면을 원본에 맞췄다.** `.sk-orb` 의 유광 스킨
//     (스페큘러·세로 그라디언트·등급색 글로우·2단 잉크 링·낙하 그림자)을 전부 걷고 **납작한 등급색
//     면 + 순검정 키라인**으로 되돌렸다(css/style.css 3925행·8131행, 원본 화소 실측 근거 첨부).
//     👉 **면 모델을 손볼 필요가 없어졌다** — 앞 메모가 남긴 숙제("최빈값 하나가 아니라 그라디언트를
//        감안한 면 모델로 빼라")는 **폐기한다.** 면이 평평해졌으므로 최빈값 한 개가 정확히 맞다.
//
//   ▶ **회복 확인(같은 마스크, 같은 자)**: 글리프를 하나도 안 그린 판의 잉크율 **16.5% → 0.0%**
//     (미장착 12개 전부 0.0%). 그리고 잉크율 격차가 **원본 34.8% ↔ 클론 34.3%(−0.5%p)** 로 닫혔다.
//     즉 세 세션이 글리프에서 찾던 20%p 는 **처음부터 오브 면이었다.**
//
//   🚨 **되돌리지 말 것** — `.sk-orb` 에 광택 층을 다시 얹으면 이 자의 전제가 같이 무너진다.
//      회귀 가드는 `tools/probe-orb-face-flat.js`(음성 대조 내장)이다. 이 자와 **같이** 돌릴 것.
//
