...
//
// 🚨🚨 **그런데 그 '닫혀 있다'는 오브 면이 평평할 때만 성립한다 — 우리 오브는 안 평평하다.**
//   (2026-08-20 UI 스트림, 음성 대조 6종으로 확정. **ⓑ 로 ㉢ 을 더 좇기 전에 이 문단을 읽을 것.**)
//   판정은 '오브 바탕 = 휘도 최빈값' 하나로 두고 **거기서 40 이상 벌어진 건 전부 모티프'라고 친다.**
//   원본은 오브 면이 **납작한 밝은 회색**이라 그 전제가 맞다. 우리 오브는 **등급색 그라디언트 +
//   좌상단 스펙큘러 하이라이트 + 바깥 글로우**라 **면 자체가 잉크로 잡힌다.**
//   ▶ 결정적 대조: `emblem()` 을 통째로 막아 **글리프를 하나도 안 그리고** 재 봤더니 —
//       잉크율 **16.5%** · ⓑ **18.6%** (평평한 면이라면 둘 다 0 에 가까워야 한다).
//     즉 우리 ⓑ 모집단의 상당 부분이 **모티프가 아니라 오브 면(거의 무채색인 하이라이트·그라디언트
//     양끝)** 이고, 그게 평균을 아래로 끌고 있다. 잉크율이 원본 34.8% ↔ 우리 55.1% 로 벌어지는 것도
//     같은 뿌리다(우리 쪽에만 면이 잉크로 섞여 든다).
//   ▶ 그래서 **-23.8%p 격차를 글리프 쪽에서 좇는 건 헛수고다.** 실제로 글리프 쪽 노브를 5개
//     극단으로 흔들어 봤는데 전부 ⓑ 를 ±2.6%p 안쪽으로만 움직였다:
//       팔레트 채도 .692 → 1.0 **+2.6** · 글로우 끔 **+2.3** · 키라인 0 **−2.2** ·
//       `floorL` 116 → 0 **0.0** · 어두운 접지 그림자 끔 **0.0**.
//   ▶ 다음에 이걸 쓰려면 **자를 먼저 고칠 것** — 오브 면을 최빈값 하나가 아니라 (반경·각도별
//     그라디언트를 감안한) 면 모델로 빼거나, 아니면 **모티프 화소를 알파 마스크로 따로 받을 것**.
//     그 전까지 **ⓑ 의 절대 격차를 '글리프가 흰색 지배'의 근거로 쓰지 말 것**(미결 ㉢).
//
