...
(async () => {
    ...
    // 게이트 1.8 — 상한이 배경 휘도에 딸려 움직인다. 글자 자리의 배경(비네트 정점의 붉은 필드 +
    // 그 뒤 3D)이 실측 0.42~0.56 사이에서 흔들리는데, **완전한 순백이라도** 대비비는 (1.05)/(bg+0.05)
    // = 2.0~2.2 가 천장이다(실측에서도 1.99·2.22 두 값이 다 나왔다). 2.0 을 걸면 배경이 조금만 밝아도
    // 통과가 불가능해진다. 이 지적의 실체는 비율이 아니라 **속살 면적**이었으므로 무게는 coreFrac 에 싣는다.
    ...
    console.log('\n  속살이 배경보다 밝은가: ' + (brighter ? '✓' : '✗') + ' (속살 ' + stat.neo.fill.toFixed(4) + ' vs 배경 ' + stat.neo.bg.toFixed(4) + ')' +
        '\n  속살:배경 ' + stat.neo.fillVsBg + ':1 (≥1.8 필요 · 이 배경 휘도에서의 천장 ' + ((1.05) / (stat.neo.bg + 0.05)).toFixed(2) + ') ' + (okBg ? '✓' : '✗') +
