...
(async () => {
    ...
    await page.waitForFunction(() => typeof Scene3D !== 'undefined' && Scene3D.heroRig && typeof Combat !== 'undefined', null, { timeout: 15000 });
    ...
    const rows = await page.evaluate(() => {
        ...
        const out = [];
        const cases = [
            ['현재 (sun1.85 h.30 r.30 e1.00 env1.0)', 1.85, 0.30, 0.30, 1.00, 1.0],
            ['라이트만 낮춤 sun1.15 h.18 r.20 e1.10 env1.0', 1.15, 0.18, 0.20, 1.10, 1.0],
            ['라이트만 낮춤 sun0.95 h.14 r.16 e1.15 env1.0', 0.95, 0.14, 0.16, 1.15, 1.0],
            ['env만 낮춤 sun1.85 h.30 r.30 e1.00 env0.5', 1.85, 0.30, 0.30, 1.00, 0.5],
            ['둘 다 sun1.15 h.18 r.20 e1.10 env0.65', 1.15, 0.18, 0.20, 1.10, 0.65],
            ['둘 다 sun0.95 h.14 r.16 e1.15 env0.5', 0.95, 0.14, 0.16, 1.15, 0.5],
        ];
