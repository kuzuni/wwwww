...
//     패널 **밖**으로 밀려 검색조차 안 된다. 행의 남색 **개수**로 보고 빈틈을 잇되, **12행까지만** 잇는다
//     — 넉넉히(40) 두면 26~27행 아래의 **하단 탭바(어두운 띠)까지 같은 패널로 삼켜** 검색 범위가
//     화면 끝까지 벌어진다(원본 실측: 패널 573~720 이 573~773 이 된다). 패널 내부 빈틈은 실측 ≤4행이다.
...
const scan = `(img) => {
    ...
    for (let y = 0; y < H; y++) {
        ...
        else if (s >= 0 && y - e > 12) { if (e - s > best) { best = e - s; bT = s; bB = e; } s = -1; }
    }
