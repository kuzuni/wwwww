...
const League = {
    ...
    checkTicketReset() {
        ...
        if (S.league.lastTicketReset !== today) {
            ...
            saveGame();
            // 매일 09:00 티켓 리셋도 checkSeasonEnd()와 동일하게 열려 있는 리그 화면에 즉시 반영
    ...
    // 1~3위 label 은 화면에 안 나온다 — UI.renderLeagueRewards 가 그 세 줄만 왕관·마름모 **배지 + 흰 숫자**로
    // 그리기 때문이다(원본 shot-042208 확대 실측). 4위 이하만 이 label 이 그대로 글자로 찍힌다.
    // 그 글자는 원본이 `4-5` 다 — '위'가 안 붙는다(원본 확대 컷에서 글자 단위 확인).
    // ⚠️ 원본 컷은 팝업이 잘려 `4-5` 줄까지만 보인다. 같은 열의 형식이 줄마다 다를 리 없어 범위 줄은
    //    전부 원본 꼴로 맞췄지만, 마지막 줄은 범위가 아니라 '위'를 떼면 `21 이하` 라는 말이 안 되는 글이
    //    되고 원본 근거도 없어 그대로 둔다 (근거가 생기기 전엔 넘겨짚지 말 것).
    REWARD_TIERS: [
        { label: '1위', rank: 1 }, { label: '2위', rank: 2 }, { label: '3위', rank: 3 },
        { label: '4-5', rank: 5 }, { label: '6-10', rank: 10 }, { label: '11-20', rank: 20 }, { label: '21위 이하', rank: 21 },
    ],
};
