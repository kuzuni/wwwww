...
const UI = {
    ...
    //    크기가 아니라 **윤곽의 비대칭**이다. 실제 외곽(아래 → 위, 반폭):
    //      🚨 접촉 모서리 11.8(빌릿 위) → **볼(챔퍼)이 13.8 로 확 벌어진다** → 몸통 8.8 → 아이 부풂 9.6 → 8.4
    //      → 🚨 **목 4.6**(y -22.6~-24.5) → 핀이 다시 8.6 으로 벌어졌다가 끝 7.0(y -36.2)
    //    🚨 **접촉 프레임 '체스 폰' 교정 (2026-08-20, 5차 채점 ㉠ — 비평가 2인 공통).** 접촉 자세(afswing
    //    20/43/73%, 회전 합 ≈0°)에서 머리가 세로 기둥이라 폰으로 읽혔다. 폰은 **밑이 좁고 위가 불룩**한데,
    //    망치는 **치는 쪽(밑)이 가장 넓어야** 한다. 그래서 접촉 모서리(11.8, 빌릿 폭 12.2 안이라 '맨 모루'로
    //    안 읽힘)는 그대로 두고, 그 **바로 위 볼을 11.6→13.8 로 확 벌려** 실루엣을 밑동이 무거운 해머 머리로
    //    만들었다. 볼은 빌릿 윗면 위(y<-0.9)라 접점 걸침을 안 키운다. probe ⑱ 의 타격면·목·핀 비는 유지.
    //    ⚠️ **핀 길이는 조형의 절반이다.** 예전엔 목(-22.6)에서 끝(-31.6)까지 9유닛뿐이라
    //    타격면→목 23.5유닛 대비 2.6:1 이었고, 두 비평가가 독립적으로 실루엣을 **체스 폰·종(鐘)**
    //    으로 읽었다. 진짜 크로스핀 해머는 핀이 몸통과 얼추 같은 길이다. 13.6유닛으로 늘려 1.7:1.
    //    (실물 비율 1:3.5:11 을 92px 에 그대로 쓸 수는 없지만, 방향은 맞춰야 어휘가 선다.)
    //    이 **잘록한 목**이 핵심이다(비평가 A 지적). 예전처럼 위로 갈수록 단조 감소하는 테이퍼면
    //    92px 에서 '망치'가 아니라 **총알·체스 폰**으로 읽힌다 — 목에서 한 번 조였다가 핀에서
    //    다시 벌어져야 눈이 '머리 + 핀'의 두 덩어리로 분절한다. probe ⑱ 이 이 잘록함을 지킨다.
    //    ⚠️ 위 수치를 고칠 때 이 주석도 같이 고칠 것 — 예전 주석이 '뭉툭한 쐐기(반폭 1.6)로
    //    끝난다'는 옛 조형을 그대로 달고 있어, 이미 들어간 목을 다시 파려던 세션이 있었다.
    ...
    HAMMER_SVG: `<g class="af-hammer">
        <g transform="translate(55,11) rotate(-20)">
            <g stroke="#14100e" stroke-width="2.2" stroke-linejoin="round" stroke-linecap="round">
                ...
                <!-- 머리 = 외곽 한 겹. 타격면 → 볼 챔퍼 → **평행 몸통** → 아이 부풂 → 짧고 뭉툭한 크로스핀 -->
                <path d="M-11.8 0.9 L11.8 0.9 Q13.6 0.9 13.8 -0.9 L13.5 -2.6 L10.2 -5.6 L8.8 -10.6 Q9.6 -12.8 8.8 -15 L8.4 -20 L4.6 -22.6 L4.6 -24.5 L8.6 -25.5 L8.6 -34.6 L7 -36.2 L-7 -36.2 L-8.6 -34.6 L-8.6 -25.5 L-4.6 -24.5 L-4.6 -22.6 L-8.4 -20 L-8.8 -15 Q-9.6 -12.8 -8.8 -10.6 L-10.2 -5.6 L-13.5 -2.6 L-13.8 -0.9 Q-13.6 0.9 -11.8 0.9 Z" fill="url(#hmr-steel)"/>
            </g>
            <g stroke="none" pointer-events="none">
                ...
                <path d="M-10.2 -5.4 L10.2 -5.4 L13.5 -2.7 L-13.5 -2.7 Z" fill="#0b0908" opacity=".36"/>
                <!-- 아이(자루 구멍) 그늘 — 자루가 머리를 관통한 것으로 읽히는 유일한 단서 -->
                <path d="M3.2 -16.4 L7.9 -16 L7.9 -9.6 L3.2 -9.2 Z" fill="#0b0908" opacity=".3"/>
                <!-- 왼쪽 림라이트: 평행 몸통을 한 줄로 훑어 금속 원통감을 만든다 -->
                <path d="M-8.9 -6.2 L-5.9 -6.2 L-5.9 -19.6 L-8.5 -19.6 Z" fill="#fff" opacity=".22"/>
                <!-- 핀 능선: 왼쪽 밝은 면 / 오른쪽 어두운 면이 만나야 '핀'이지 '못'이 아니다 -->
                <path d="M-8.2 -26.1 L-5.6 -26.1 L-5.6 -34.9 L-8.2 -34.4 Z" fill="#fff" opacity=".28"/>
                <path d="M4.4 -26.1 L8.2 -26.1 L8.2 -34.4 L4.4 -34.9 Z" fill="#000" opacity=".26"/>
                <!-- 자루 나뭇결 하이라이트 -->
