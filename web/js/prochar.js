...
const ProChar = {
    ...
    CLIPS: {
        ...
        // 2단 붕괴: ① 피격 반동 + 무릎 꺾여 주저앉음 → ② 무게가 실려 뒤로 쿵 넘어져 등이 바닥에 붙음.
        // ⚠️ 지면(y≈0) 접촉이 핵심: 이전 버전은 root.rx가 −1.35(77°)까지만 젖혀지고 root.py를 −0.44까지
        //   묻어, 골반은 지면 아래로 파묻히는데 상체·팔은 공중에 뜬 '비스듬히 누운' 실루엣이었다(사용자 재지적).
        //   → root.rx를 −1.55(거의 평평)까지 눕히고, root.py 최종값은 리그 최저 world Y가 지면에 살짝 닿도록
        //   실측(probe-death-ground.js)으로 잡은 −0.02. 다리도 최종엔 펴서 몸 전체가 바닥에 평평히 눕는다.
        Death: {
            dur: 1.3, once: true, tracks: {
                // ── 상체: 피격 반동(뒤로 움찔) → 앞으로 무너짐 → 뒤로 넘어가며 등이 바닥에, 마지막에 가슴 살짝 펴짐
                'spine.rx': [[0, 0], [0.12, -0.28], [0.32, 0.22], [0.6, -0.05], [1, 0.12]],
                'spine.ry': [[0, 0], [0.18, 0.32], [0.55, 0.1], [1, 0.06]],
                'neck.rx': [[0, 0], [0.2, 0.25], [0.55, 0.15], [0.78, -0.28], [1, -0.12]], // 고개 떨궜다 → 넘어가며 뒤로
                // ── 다리: 무릎 꺾여 주저앉음(0.3) → 넘어가며 앞으로 뻗어 바닥에 평평히
                'hipL.rx': [[0, 0], [0.3, 0.9], [0.55, 0.5], [1, 0.08]],
                'hipR.rx': [[0, 0], [0.3, 0.78], [0.55, 0.42], [1, 0.04]],
                'kneeL.rx': [[0, 0], [0.3, -1.15], [0.55, -0.55], [1, -0.1]],
                'kneeR.rx': [[0, 0], [0.3, -1.02], [0.55, -0.48], [1, -0.06]],
                // ── root: 주저앉으며 살짝 내려감(0.3) → 뒤로 쿵 넘어가며 등이 지면에 닿게 눕힘
                //   root.rx 0.72에서 −1.62로 살짝 오버슈트 후 −1.55로 안착(등이 바닥 치는 반동)
                'root.rx': [[0, 0], [0.18, 0.08], [0.32, -0.05], [0.55, -0.78], [0.72, -1.62], [0.85, -1.5], [1, -1.55]],
                //   root.py: 주저앉음 −0.14 → 뒤로 넘어가 등 접지 지점(−0.02), 접지 순간 미세 바운스
                'root.py': [[0, 0], [0.3, -0.14], [0.55, -0.1], [0.72, 0.02], [0.82, -0.05], [1, -0.02]],
                // ── 팔: 균형 잃고 벌어졌다가 몸 옆으로 툭 떨어져 바닥에 눕는다
                'shoulderL.rx': [[0, -0.1], [0.3, -0.5], [0.6, 0.05], [1, -0.12]],
                'shoulderR.rx': [[0, -0.14], [0.3, -0.55], [0.6, 0.08], [1, -0.1]],
                'shoulderL.rz': [[0, 0], [0.4, 0.35], [1, 0.12]],   // 팔이 옆으로 벌어져 바닥에
                'shoulderR.rz': [[0, 0], [0.4, -0.35], [1, -0.12]],
                'elbowL.rx': [[0, 0], [0.35, -0.6], [1, -0.15]],
                'elbowR.rx': [[0, 0], [0.35, -0.55], [1, -0.12]],
            }
        },
        // 기상: 쓰러진 포즈(Death 최종값)에서 그대로 출발해 상체를 세우고 다리를 접어 일어선다.
        // ⚠️ 시작 키프레임은 새 Death 끝값과 정확히 일치해야 순간이동이 없다.
        Revive: {
            dur: 0.8, once: true, tracks: {
                'root.rx': [[0, -1.55], [0.4, -0.75], [0.8, 0.12], [1, 0]],   // 등을 떼고 상체를 먼저 일으키고
                'root.py': [[0, -0.02], [0.35, -0.16], [0.7, -0.05], [1, 0]], // 무릎을 접었다 펴며 키가 돌아온다
                'hipL.rx': [[0, 0.08], [0.4, 0.95], [0.7, 0.6], [1, 0]],
                'hipR.rx': [[0, 0.04], [0.4, 0.88], [0.7, 0.55], [1, 0]],
                'kneeL.rx': [[0, -0.1], [0.4, -1.2], [0.7, -0.7], [1, 0]],
                'kneeR.rx': [[0, -0.06], [0.4, -1.1], [0.7, -0.65], [1, 0]],
                'spine.rx': [[0, 0.12], [0.5, 0.28], [0.85, -0.1], [1, 0]],   // 마지막에 가슴을 살짝 펴는 오버슈트
                'spine.ry': [[0, 0.06], [1, 0]],
                'neck.rx': [[0, -0.12], [0.5, 0.1], [1, 0]],
                'shoulderL.rx': [[0, -0.12], [1, -0.1]],
                'shoulderR.rx': [[0, -0.1], [1, -0.14]],
                'shoulderL.rz': [[0, 0.12], [1, 0]],
                'shoulderR.rz': [[0, -0.12], [1, 0]],
                'elbowL.rx': [[0, -0.15], [0.5, -0.4], [1, 0]],
                'elbowR.rx': [[0, -0.12], [0.5, -0.35], [1, 0]],
            }
        },
    },
