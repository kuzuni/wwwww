    ...
    CLIPS: {
        ...
        slash: { // 대각 베기: 와인드업(뒤·위) → 스냅 스윙 → 팔로스루
            dur: 0.5, tracks: {
                ...
                'neck.ry': [[0, 0], [0.3, 0.55], [0.55, -0.49], [1, 0]],
                ...
                // 🚨 **접촉 창(0.45~0.6)에서는 척추 배율을 정확히 1.0 으로 둔다** — 여기를 만지면 어깨→팔→무기
                //    끝이 딸려 움직여 `probe-impact-sync`(무기 끝 접촉 시각 vs 임팩트 연출)가 어긋난다.
                //    스쿼시는 **와인드업(0~0.35)과 팔로스루 착지(0.65~1.0)에만** 얹는다.
                // 와인드업 = 몸을 뒤·위로 세워 코일(stretch), 스윙 직전 0.4 에서 1.0 복귀 → 휘두르며 아래로.
                'spine.sy': [[0, 1], [0.18, 1.055, 'back'], [0.3, 1.05], [0.45, 1.0], [0.6, 1.0], [0.78, 0.955, 'bounce'], [1, 1, 'back']],
                'spine.sx': [[0, 1], [0.18, 0.97], [0.3, 0.975], [0.45, 1.0], [0.6, 1.0], [0.78, 1.03], [1, 1]],
                'spine.sz': [[0, 1], [0.18, 0.97], [0.3, 0.975], [0.45, 1.0], [0.6, 1.0], [0.78, 1.03], [1, 1]],
                // 머리는 반대 위상(카툰 대비) — 와인드업에 눌리고 착지에 늘어난다. 접촉 창은 1.0.
                'head.sy': [[0, 1], [0.18, 0.955], [0.3, 0.96], [0.45, 1.0], [0.6, 1.0], [0.78, 1.045], [1, 1]],
                'head.sx': [[0, 1], [0.18, 1.03], [0.3, 1.025], [0.45, 1.0], [0.6, 1.0], [0.78, 0.97], [1, 1]],
            }
        },
        chop: { // 머리 위로 크게 들어 내려찍기
            dur: 0.55, tracks: {
                ...
                'neck.ry': [[0, 0], [0.35, 0.21], [0.6, -0.17], [1, 0]],   // 시선 전방 유지 (slash 주석 참조)
                'kneeL.rx': [[0, 0], [0.6, -0.25], [1, 0]],
                'cape.rx': [[0, 0.05], [0.5, 0.55], [1, 0.05]],
                // 예비동작·팔로스루 스쿼시 (slash 주석 참조 — 접촉 창 0.5~0.7 은 1.0 고정). 접촉 ~0.6.
                'spine.sy': [[0, 1], [0.22, 1.06, 'back'], [0.35, 1.05], [0.5, 1.0], [0.7, 1.0], [0.85, 0.95, 'bounce'], [1, 1, 'back']],
                'spine.sx': [[0, 1], [0.22, 0.968], [0.35, 0.975], [0.5, 1.0], [0.7, 1.0], [0.85, 1.03], [1, 1]],
                'spine.sz': [[0, 1], [0.22, 0.968], [0.35, 0.975], [0.5, 1.0], [0.7, 1.0], [0.85, 1.03], [1, 1]],
                'head.sy': [[0, 1], [0.22, 0.95], [0.35, 0.96], [0.5, 1.0], [0.7, 1.0], [0.85, 1.05], [1, 1]],
                'head.sx': [[0, 1], [0.22, 1.032], [0.35, 1.025], [0.5, 1.0], [0.7, 1.0], [0.85, 0.968], [1, 1]],
            }
        },
        thrust: { // 찌르기: 당겼다 런지
            dur: 0.45, tracks: {
                ...
                'neck.ry': [[0, 0], [0.3, -0.3], [0.55, 0.38], [1, 0]],   // 시선 전방 유지 — 특히 찌르는 순간(−0.45 비틀림)에 얼굴이 뒤로 가면 안 된다 (slash 주석 참조)
        ...
        double: { // 좌우 연속 베기
            dur: 0.55, tracks: {
                ...
                'neck.ry': [[0, 0], [0.25, 0.34], [0.5, -0.3], [0.75, 0.26], [1, 0]],   // 시선 전방 유지 (slash 주석 참조)
