...
(function (G) {
    ...
    G.draw.tab_dungeon = function (ctx, S) {
        ...
        ink(ctx, S, circle(ctx, S, P0[0], P0[1], 0.042), '#f07000', 0.042);                 // 폼멜(주황 — 회색이면 '돋보기'로 오독)
    ...
    G.draw.tab_summon = function (ctx, S) {
        ...
        // 불꽃 혀: 큰 혀는 링 위 중앙에서 위로, 작은 혀는 오른쪽에서 비스듬히 — 곡면 목으로 이어
        // 삼각 스파이크(화살촉)와 구별되게 한다.
        ink(ctx, S, () => {
            ctx.moveTo(0.520 * S, 0.212 * S);
            ctx.bezierCurveTo(0.530 * S, 0.120 * S, 0.556 * S, 0.086 * S, 0.586 * S, 0.036 * S);
            ctx.bezierCurveTo(0.606 * S, 0.108 * S, 0.652 * S, 0.140 * S, 0.664 * S, 0.212 * S);
            ctx.closePath();
        }, G1, 0.052);
        ink(ctx, S, () => {
            ctx.moveTo(0.700 * S, 0.238 * S);
            ctx.bezierCurveTo(0.722 * S, 0.176 * S, 0.756 * S, 0.156 * S, 0.792 * S, 0.128 * S);
            ctx.bezierCurveTo(0.794 * S, 0.190 * S, 0.816 * S, 0.220 * S, 0.802 * S, 0.286 * S);
            ctx.closePath();
        }, G1, 0.048);
        ...
        // 스페큘러 — 재채점 반영(2인 공통 '광택 과다·클립아트 질감'): 방사형 그라디언트를 걷고
        // 원본의 플랫 셀 셰이딩 문법대로 **작은 단색 점 하나**만 남긴다.
        on(ctx, ell(ctx, S, 0.596, 0.578, 0.052, 0.040, -0.5), 'rgba(255,255,255,.82)');
    };
    ...
    G.draw.tab_pvp = function (ctx, S) {
        ...
        // 재채점 반영(비평가 2인 공통): 검마다 코등이 끝 구슬 2개를 달았더니 하단 중앙에 금색 원
        // 6개가 모여 '검'이 아니라 '발자국 점무늬'로 읽혔다 — 구슬은 자루 끝 1개만 남기고,
        // 검 자체를 키워('작고 하단으로 몰림' 지적) 크로스가드·그립·폼멜이 각각 판독되게 한다.
        const sword = (hx, hy, tx, ty) => {
            ...
            ink(ctx, S, poly(ctx, S, [[-0.108, u(0.185)], [0.108, u(0.185)], [0.108, u(0.115)], [-0.108, u(0.115)]]), '#ffb200', 0.040); // 코등이 = 긴 바 하나(끝 구슬 없음)
            ink(ctx, S, poly(ctx, S, [[-0.026, u(0.11)], [0.026, u(0.11)], [0.026, u(0.005)], [-0.026, u(0.005)]]), '#7a4a12', 0.038); // 그립(짙은 갈색 — 가드와 톤 분리)
            ink(ctx, S, circle(ctx, S, 0, u(-0.035), 0.036), '#ffb200', 0.038);                // 폼멜(검당 1개)
            ctx.restore();
        };
        sword(0.368, 0.732, 0.664, 0.360);
        sword(0.632, 0.732, 0.336, 0.360);
