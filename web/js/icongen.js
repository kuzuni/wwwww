...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        [0.90, 0.60], [0.90, 0.50], [0.95, 0.46], [1.02, 0.50]],
            '#a8cc63', '#83aa53');   /* R4: 중경 열 올리브그린 복귀(원본 정합 — 2인 치명 '회색=다른 게임'. 병렬 세션 R3 의 '담장 판독' 처방(지붕 2톤+창)은 아래에 올리브 팔레트로 이식해 병합) */
        ...
        (() => {
            ...
            for (let i = 0; i < 4; i++)
                blobs.push([W * 0.79 + H * (-0.34 + R() * 0.40), H * (-0.12 + R() * 0.26), H * (0.12 + R() * 0.08), '#2a7029']);
        ...
        bush(W * 0.30, H * 1.06, 1.0, '#519f36', '#16391a');
        // 좌상단 늘어진 수풀 — 원본은 좌우 가장자리가 잎으로 채워져 프레임이 꽉 찬다(비평가 4차 '성김')
        for (let i = 0; i < 7; i++) {
            const x = W * (-0.015 + R() * 0.115), y = H * (-0.08 + R() * 0.32), r = H * (0.105 + R() * 0.085);
        ...
        ctx.translate(1.735 * S, 0.826 * S); ctx.rotate(SWING + Math.PI / 2); ctx.scale(0.86, 0.86);
        ...
        on(ctx, rrect(ctx, S, -0.135, 0.620, 0.270, 0.318, 0.060), 'rgba(0,0,0,.14)');    // 팔뚝 하단 그늘(명암 한 단)
        ctx.restore();
        ctx.save();                                                                       // 머리 — 자루 끝(우상단)
        // 머리 상자는 세로(local y)가 장축 = 자루와 직교하려면 회전각이 자루 기울기 그대로여야 한다.
        ctx.translate(2.075 * S, 0.510 * S); ctx.rotate(SWING); ctx.scale(0.70, 0.70);
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        /* 달빛 헤이즈 — 원본 상단의 그 한 단 밝은 하늘 덩어리(플랫 타원 한 장, 그라디언트 금지
           규약 유지). 좌측 제목 자리는 안 건드린다(title-ink 좌반 하한 26). */
        fill(ctx, '#333c5e', () => ctx.ellipse(W * 0.56, H * 0.085, W * 0.30, H * 0.115, 0, 0, Math.PI * 2));
        ...
        const stone = (x, y, w, h, sq) => {                                // sq=true 면 각진 어깨(형태 리듬 — 비평가 5차)
            ...
            ctx.beginPath(); p(); ctx.fillStyle = '#1e2942'; ctx.fill();   // R8 값게이트: 카드 하단 밴드가 원본보다 +24 떴다 — 반 단 침강   // R7: 채움을 반 단 올려 근흑 키라인이 갈리게(무선 오독 해소 — '밝다' 지적과의 균형점)   // R5 A·B 재공통 '묘비 스티커처럼 팝' — 한 단 더 침강(각인 자체는 원본 실측 사양: 해골+RIP 유지)
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        (() => {
            ...
            ctx.lineTo(W, H); ctx.closePath(); ctx.fillStyle = '#9c8570'; ctx.fill();   // R8 값게이트: 원경 띠도 원본 밴드(102)에 맞춰 침강
        ...
        const MIDC_HI = ['#544038', '#4b3830', '#5c463c', '#503c32'];   // R8 값게이트: 원본 중경 실측 #403030(L52) 대역
        const MIDC = ['#33231e', '#2c1e1a', '#38271f', '#2f211c'];      // R8 값게이트: 원본 전경 실측 #201010(L21) 직전 단         // 색·키·크기를 함께 흔들어 타일링 패턴을 깬다
        ...
        banner(W * 0.30, H * 0.11, H * 0.24, H * 0.26, -0.20, '#4a3527');  // R5 B '원본은 검정 깃발': 밝은 갈색 2장 근흑화(암적 악센트 2장은 3차 사양 유지)
        banner(W * 0.585, H * 0.06, H * 0.22, H * 0.24, -0.14, '#3e2b20');
        banner(W * 0.05, H * 0.05, H * 0.25, H * 0.29, 0.06, '#3a1712');   // R6 B(3연속) '원본은 검정 위주' — 암적 악센트도 근흑 쪽으로   /* 9차 '배너 복붙' — 색을 4종으로 갈랐다 · 3차 교집합 ⓔ: 1장은 암적 악센트(전장 서사 + 팔레트 포인트) */
        banner(W * 0.14, -H * 0.02, H * 0.30, H * 0.32, 0.10, '#2e1c15');
        banner(W * 0.46, -H * 0.06, H * 0.28, H * 0.30, -0.08, '#241511');
        banner(W * 0.70, H * 0.01, H * 0.27, H * 0.30, 0.16, '#2f130c');  // 3차 교집합 ⓔ: 암적 두 번째(좌우로 벌려 반복감 방지)
        (() => {                                                           // 지면 먼지/헤이즈 — 하단이 딱딱하게 끊기지 않게(비평가 3차)
            ...
            ctx.fillStyle = hz; ctx.fillRect(0, H * 0.40, W, H * 0.32);
        })();
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        sky(ctx, W, H, '#e4d2fb', '#cbb2e8');   // R8 값게이트: 하단이 #8a63b5(L115)면 0.3~0.6H 가 원본보다 40 어둡다
        ...
        const tree = (x, sc) => {
            ...
            /* R5 A '중간 명도 보라 나무 — 원본은 거의 검은 자주 실루엣': 파스텔 하늘 전환(R4)으로
               상대 명도가 떠서 한 단 침강 */
            const c = '#3a2751', oc = '#0d0716', u = H * sc, B = H * 0.86;   // R7: 채움·키라인 명도 분리(둘 다 근흑이면 무선으로 읽힌다)
            const P = [];
        ...
        tree(W * 0.085, 0.92); tree(W * 0.64, 1.15);
