    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        const figure = (x, y, s, c, kind, oc, wf, ow) => {
            ...
            /* R3 B ⓓ '자갈밭 — pickKind 6종이 실루엣 단계에서 안 갈린다': 부속(꼭지·챙·볏·뿔)을
               ...
               빈도(8%)는 유지. */
            if (kind === 1) paths.push(() => { ctx.moveTo(x - r, y + r * 0.5); ctx.lineTo(x, y - r * 1.18); ctx.lineTo(x + r, y + r * 0.5); ctx.closePath(); });   // R9 D '정체불명 삼각형': 전경 배율에서 꼭짓점이 너무 높아 고깔 천막으로 읽혔다 — 한 단 낮춤
        ...
        for (let i = 0; i < 9; i++) {
            ...
            figure(W * (i / 9 + 0.012 + R() * 0.030), H * (0.700 + R() * 0.052), 2.18 + R() * 0.58, i % 4 ? '#221208' : '#2d1a0e', [6, 2, 4, 6, 8, 6, 5, 1][(R() * 8) | 0], '#050302', 1.9 + R() * 0.8, H * 0.026);   // R9 2인 공통: 종 6종을 8종으로(뿔·어깨 날붙이 추가) + 크기 편차 +-15%   // R6 B '전경 틈 제거'(밀도·어깨폭 업) · 머리가 하단 크롭에 잘리면 삼각 톱니로 읽힌다 — 0.875H · R2 공통: 전경 키라인 굵기 통일
        }
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        const band = (bx, by, bw, bh2, c) => {
            const p = () => {
                ...
                ctx.arc(bx + bw * 0.32, by + bh2 * 0.14, bh2 * 1.05, 0, Math.PI * 2, true);   // 혹 — R9 2인 공통 '동글동글 뭉게구름': 납작하게 낮춘다
                ctx.arc(bx + bw * 0.70, by + bh2 * 0.24, bh2 * 0.80, 0, Math.PI * 2, true);
            };
        ...
        band(W * 0.095, H * 0.15, W * 0.205, H * 0.042, '#a683c8');   // R9: 가로로 길고 납작하게(원본의 수평 구름 띠)
        band(W * 0.44, H * 0.10, W * 0.165, H * 0.038, '#9d7ac0');
        band(W * 0.325, H * 0.265, W * 0.130, H * 0.032, '#a683c8');
