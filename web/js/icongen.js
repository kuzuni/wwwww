...
const IconGen = {
    ...
    draw: {
        ...
        egg(ctx, S, o) {
            ...
            // 🚫 **좌우 뿔(로브)을 여기에 얹으려다 되돌렸다 — 다음 세션이 같은 길로 다시 가지 말 것.**
...
(function (G) {
    ...
    G._EMBLEM_STEP = { top: 0.24, mid: 0.02, bot: -0.26, floorL: 116, gloss: 0.18, keyline: 0.067 };
    ...
    const P = {
        ...
        cleaver(ctx, S) {                                // 처형 — 처형대 칼날 (fx guillotineDrop)
            ...
            ctx.moveTo(x(.10, S), y(.10, S)); ctx.lineTo(x(.90, S), y(.10, S));
            ctx.lineTo(x(.90, S), y(.38, S)); ctx.lineTo(x(.10, S), y(.66, S));
            ctx.closePath();
            // ⓑ 형틀(받침대) — 목이 놓이는 **반달 홈**을 크게 판 두꺼운 막대. 날과 **떨어뜨려** 둔다:
            //    그 빈 틈이 '아직 안 떨어졌다'를 만들고, 동시에 서브패스가 안 겹쳐 이음매 검은 줄도 없다.
            ctx.moveTo(x(.10, S), y(.72, S)); ctx.lineTo(x(.36, S), y(.72, S));
            ctx.lineTo(x(.36, S), y(.855, S)); ctx.lineTo(x(.64, S), y(.855, S));
            ctx.lineTo(x(.64, S), y(.72, S)); ctx.lineTo(x(.90, S), y(.72, S));
            ctx.lineTo(x(.90, S), y(.95, S)); ctx.lineTo(x(.10, S), y(.95, S));
            ctx.closePath();
        },
