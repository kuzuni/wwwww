...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        sky(ctx, W, H, '#a5d2f8', '#cfe9fc');   // R6 B '하늘이 씻겨 보임' — 하단 연청 복원
        ...
        for (let i = 0; i < 14; i++) {                                    // 풀포기 — R6 2인 공통 '등간격 스탬프/울타리 띠': 수 절반 + 폭·높이 편차
            ...
            fill(ctx, '#3d7d2c', () => { ctx.moveTo(x, y); ctx.lineTo(x + w2 * (0.35 + R() * 0.3), y - h); ctx.lineTo(x + w2, y); });
        }
        ...
        on(ctx, poly(ctx, S, [FR[1], FR[2], [0.115 + VX, 0.255 + VY], [0.115 + VX, -0.255 + VY]]), '#6f7639');   // 윗면(빛면)
        ...
        ink(ctx, S, rrect(ctx, S, -0.095, 0.245, 0.120, 0.090, 0.022), '#b08a3c', 0.026);   // 자루 물림 금 테(R6 A — 원본 실측 '올리브+금'의 금 단서)
        ctx.restore();
    };
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        const stone = (x, y, w, h, sq) => {                                // sq=true 면 각진 어깨(형태 리듬 — 비평가 5차)
            ...
            ctx.beginPath(); p(); ctx.strokeStyle = '#05071a'; ctx.lineWidth = H * 0.042; ctx.stroke();   // R2 공통 '전경 키라인 비일관' — 굵은 근흑 키라인(스트로크→채움 순서로 바깥만 남긴다)
            ctx.beginPath(); p(); ctx.fillStyle = '#233150'; ctx.fill();   // R5 A·B 재공통 '묘비 스티커처럼 팝' — 한 단 더 침강(각인 자체는 원본 실측 사양: 해골+RIP 유지)
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        const pickKind = k =>                                              // 6종 혼합(ⓐ) — 뾰족·뿔은 드물게(9차 '삼각형 정체불명')
            k < 0.28 ? 0 : k < 0.42 ? 1 : k < 0.62 ? 3 : k < 0.86 ? 4 : k < 0.96 ? 5 : 2;   // R6 A '중경 사각 블록': 챙(5) 빈도 축소
        ...
        banner(W * 0.05, H * 0.02, H * 0.30, H * 0.34, 0.06, '#3a1712');   // R6 B(3연속) '원본은 검정 위주' — 암적 악센트도 근흑 쪽으로   /* 9차 '배너 복붙' — 색을 4종으로 갈랐다 · 3차 교집합 ⓔ: 1장은 암적 악센트(전장 서사 + 팔레트 포인트) */
        banner(W * 0.14, -H * 0.10, H * 0.40, H * 0.42, 0.10, '#2e1c15');
        banner(W * 0.46, -H * 0.14, H * 0.38, H * 0.40, -0.08, '#241511');
        banner(W * 0.70, -H * 0.05, H * 0.36, H * 0.40, 0.16, '#2f130c');  // 3차 교집합 ⓔ: 암적 두 번째(좌우로 벌려 반복감 방지)
        ...
        (() => {
            /* R3 A3 '키라인이 안 보인다(근흑 채움 위 검정 키라인)': 채움을 한 단 밝혀 키라인이
               실제로 갈리게 한다. */
            const x0 = W * 0.335, FC = '#2c1a0d';   // R6 A '살구빛 마네킹' ↔ 병렬 R3 '키라인 가시화' 절충: 중간 명도
            const paths = [];
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        const drawRemains = () => {
            ...
            ctx.strokeStyle = '#150e26'; ctx.lineWidth = H * 0.036;
            ...
            ctx.strokeStyle = '#150e26'; ctx.lineWidth = H * 0.028;
