    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        (() => {
            ...
            for (let i = 0; i < 9; i++)                                    // 잎 덩어리(자리 먼저 뽑는다 — 스트로크→채움 2패스용)
                blobs.push([x + H * (-0.22 + R() * 0.50), H * (-0.08 + R() * 0.30), H * (0.12 + R() * 0.08), i % 2 ? '#265034' : '#2d5c40']);   // 이번 런 B: 하늘과 값이 붙어 실루엣 소실 — 1.5스텝 밝게
            ...
            for (let i = 0; i < 5; i++) {                                  // 달빛 림 혹
                ...
                fill(ctx, '#3f7248', () => ctx.arc(lx, ly, r, 0, Math.PI * 2));   // 달빛 림도 한 단 위로
            }
        })();
