...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        [0.90, 0.60], [0.90, 0.50], [0.95, 0.46], [1.02, 0.50]],
            '#8fae62', '#6f9150');   /* R4: 중경 열 올리브그린 복귀(원본 정합 — 2인 치명 '회색=다른 게임'. 병렬 세션 R3 의 '담장 판독' 처방(지붕 2톤+창)은 아래에 올리브 팔레트로 이식해 병합) */
        ...
        [0.475, 0.505, 0.535, 0.30, 0.24], [0.63, 0.665, 0.70, 0.42, 0.36], [0.76, 0.80, 0.84, 0.48, 0.42]].forEach(([lx, px, rx2, by, py]) =>
            fill(ctx, '#7b9c55', () => { ctx.moveTo(W * lx, H * by); ctx.lineTo(W * px, H * py); ctx.lineTo(W * rx2, H * by); }));
        [[0.048, 0.50], [0.178, 0.42], [0.328, 0.50], [0.498, 0.37], [0.655, 0.48], [0.792, 0.52]].forEach(([wx, wy]) =>
            fill(ctx, '#6f9150', () => ctx.rect(W * wx, H * wy, W * 0.014, H * 0.032)));
        ...
        ground(ctx, W, H, H * 0.82, '#75b243');
        ground(ctx, W, H, H * 0.91, '#55a034');
        for (let i = 0; i < 26; i++) {                                    // 풀포기 — R4: 지면과 함께 황록으로(원본 실측 145,188,69)
            const x = R() * W, y = H * (0.84 + R() * 0.14), h = H * (0.05 + R() * 0.05);
            fill(ctx, '#3d7d2c', () => { ctx.moveTo(x, y); ctx.lineTo(x + H * 0.02, y - h); ctx.lineTo(x + H * 0.04, y); });
        }
        ...
        ink(ctx, S, rrect(ctx, S, -0.135, 0.238, 0.270, 0.580, 0.060), '#c88a58', 0.030); // 팔뚝(맨팔) — 프레임 하단 크롭 밖까지
        on(ctx, rrect(ctx, S, -0.135, 0.560, 0.270, 0.258, 0.060), 'rgba(0,0,0,.14)');    // 팔뚝 하단 그늘(명암 한 단)
        ctx.restore();
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        const stone = (x, y, w, h, sq) => {                                // sq=true 면 각진 어깨(형태 리듬 — 비평가 5차)
            ...
            ctx.beginPath(); p(); ctx.fillStyle = '#31405f'; ctx.fill();   // R4 비평가 2인 공통 '묘비가 여전히 팝': 6차의 한 단에 이어 한 단 더 — 어두운 남색 쪽으로
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        const MIDC_HI = ['#8f7060', '#836253', '#997e70', '#886a55'];
        const MIDC = ['#755749', '#68493c', '#7f6055', '#6d503e'];         // 색·키·크기를 함께 흔들어 타일링 패턴을 깬다
        ...
        for (let i = 0; i < 13; i++) {
            ...
            figure(W * (i / 13 + 0.015 + R() * 0.025), H * (0.875 + R() * 0.08), 1.75 + R() * 0.8, i % 4 ? '#2b1a12' : '#38251a', [6, 6, 4, 6, 5, 1][(R() * 6) | 0], '#050302', 1.6 + R() * 0.7, H * 0.026);   // 머리가 하단 크롭에 잘리면 삼각 톱니로 읽힌다 — 0.875H · R2 공통: 전경 키라인 굵기 통일
        }
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        /* R4 비평가 B '원본은 밝은 라벤더(실측 204,177,255) — 클론은 밤처럼 어둡다': 파스텔로 올린다 */
        sky(ctx, W, H, '#d3b2f0', '#8a63b5');
        ...
        /* R4 비평가 A '외곽선 없는 소프트 블롭 — 키라인 원칙 위반': 구름에도 합집합 키라인
           (스트로크→채움 2패스, 덤불 화법). 하늘이 파스텔로 밝아져 채움도 한 단 진하게 갈랐다. */
        /* ⚠️ 키라인을 두르는 순간 '가는 캡슐 + 작은 혹'은 '구슬 꿴 철사'로 읽힌다(이번 런 캡처
           확인) — 밴드를 짧고 두껍게, 혹 반경을 밴드 두께의 1.3~1.9배로 키워 뭉게 덩어리로. */
        const band = (bx, by, bw, bh2, c) => {
            const p = () => {
                ...
                ctx.closePath();
                /* 혹 원은 캡슐과 같은 반시계 감김으로 — 감김이 갈리면 nonzero 규칙에서 겹침이
                   구멍이 되어 캡슐 윗변 스트로크가 혹 안으로 비쳐 '선 관통'이 된다(캡처 확인). */
                ctx.arc(bx + bw * 0.32, by + bh2 * 0.10, bh2 * 1.90, 0, Math.PI * 2, true);   // 뭉게 혹(왼쪽 크게)
                ctx.arc(bx + bw * 0.68, by + bh2 * 0.22, bh2 * 1.30, 0, Math.PI * 2, true);   // 뭉게 혹(오른쪽 작게)
            };
            ctx.beginPath(); p(); ctx.strokeStyle = '#6c4a99'; ctx.lineWidth = H * 0.018; ctx.stroke();
            fill(ctx, c, p);
        };
        band(W * 0.115, H * 0.15, W * 0.115, H * 0.052, '#e6d2f2');
        band(W * 0.45, H * 0.10, W * 0.085, H * 0.046, '#dfc6ee');
        band(W * 0.345, H * 0.26, W * 0.065, H * 0.040, '#e6d2f2');
        ...
        tree(W * 0.10, 1.05); tree(W * 0.64, 1.15);
