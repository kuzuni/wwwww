...
const IconGen = {
    ...
    BLOCK: {
         ...
         *    ✅ **결정 = ⓑ (2026-08-25 UI 스트림). '4종만 위한 특례'라는 이유로 미룰 자리가 아니었다.**
         *       앞 세션이 ⓑ 를 안 고른 근거는 "특례라서"인데, **이 파일은 이미 같은 4종을 같은
         *       이유로 특례 처리하고 있다** — 바로 아래 `MODE_SKIP: ['dg_']` 이고, 그 주석의 사유가
         *       *"던전 배너는 아이콘이 아니라 **연속톤 풍경화**"* 다. 두 톤짜리 어두운 면이 필요한
         *       것이 정확히 **연속톤 풍경화의 성질**이므로, ⓑ 는 새 분류축을 지어내는 게 아니라
         *       **이미 파일에 적힌 분류를 한 번 더 쓰는 것**이다. ⑷ 의 자동 판별축 셋이 실패한 것도
         *       같은 이야기다 — 배너와 문장을 가르는 건 화소 통계가 아니라 **그림의 종류**이고,
         *       그 판단은 사람이 이미 내려 `MODE_SKIP` 에 적어 두었다.
         *       그래서 `SCENE_ART`(아래)는 `MODE_SKIP` 과 **같은 목록을 일부러 공유하지 않고
         *       따로 둔다** — 두 노브가 뜻하는 바가 다르고(칸 색 평균 vs 바닥 두 톤), 한쪽만
         *       바꾸고 싶은 날이 오면 같은 배열을 공유한 쪽이 조용히 깨진다. */
        ...
        LCEIL: 0.875,
        LCEIL_C: 0.12,
        /* `LFLOOR` 의 **두 번째 바닥** — 바닥 버킷(l 이 0 으로 반올림된 칸)만 반 칸 더 내린다.
         * 🚨 **전 종에 걸면 `probe-emblem-core` 가 3종 깨진다**(위 `LFLOOR` 갈림길 ⑶ 참조) —
         *    그래서 `SCENE_ART` 목록에 든 종에만 적용한다. 값 0.125 는 버킷 사이의 정확히
         *    한가운데라 팔레트를 한 색만 늘린다(0.25 와 0.125 두 톤). 더 내리면(0.0625) 두 톤이
         *    붙어 다시 한 색으로 읽히고, 0 으로 두면 `LFLOOR` 자체가 사라진 것과 같아진다. */
        LFLOOR_DEEP: 0.125,
        /* 🖼 **프레임을 가득 채우는 장면 그림**인 종 — 지금은 가로 3.45:1 던전 배너 4종뿐이다.
         * 아이콘은 **투명 바탕에 뜬 실루엣**이고 배너는 **잘라 낸 그림 한 장**이라, 블록화의 두
         * 규칙이 배너에서만 반대로 작동한다. 그 둘을 여기 한 목록으로 묶는다:
         *  ⑴ **바닥을 둘로 쪼갠다**(`LFLOOR_DEEP`). 아이콘은 어두운 속살이 한 톤이어도 실루엣이
         *     읽히지만, 장면 그림은 어두운 층(원경·근경)이 여러 겹이라 한 톤으로 뭉치면 층이
         *     통째로 사라진다. 위 `LFLOOR` 갈림길 주석의 ✅ 항이 결정 경위다.
         *  ⑵ **캔버스 테두리를 실루엣 경계로 안 본다**(`edge()` 의 `SCENE`). 아이콘은 실루엣이
         *     테두리에 닿으면 거기가 진짜 경계지만, 장면 그림에서 테두리는 **자른 자리**다 —
         *     거기에 키라인을 찍으면 원본에 없는 **검은 액자**가 둘린다. 실측(2026-08-25
         *     `dg-band-luma`): 20칸 격자에서 맨 아랫줄 한 칸 = 세로 5% 인데 밴드 하나가 8.3% 라,
         *     그 한 줄이 밴드11 의 **59.6%** 를 순검정으로 채웠다(`dg_zombie` 밴드11 원본 49.6 대
         *     클론 24.5 = Δ−25.1 의 정체가 이것이다. 지면 색을 아무리 손봐도 안 움직인다).
         * ⚠️ 여기에 아이콘 종(`sk_*`·`age_*` 등)을 추가하지 말 것 — ⑴ 쪽에서 `probe-emblem-core`
         *    가 '속살 = 루마 ≥ 50' 으로 문장 24종을 물고 있고, 그 통과가 `LFLOOR`(0.25) 의
         *    들어올림에 기대고 있다는 것이 실측으로 확인됐다(전 종 적용 시 3종 미달).
         * 📌 아래 `MODE_SKIP` 과 목록이 같지만 **뜻이 다르므로 배열을 공유하지 않는다**
         *    (칸 색 화법 vs 층 구조. 한쪽만 바꾸고 싶은 날 공유한 쪽이 조용히 깨진다). */
        SCENE_ART: ['dg_'],
     ...
     *    🔬 **`BLOCK_CELLS: { avatar_: 40 }` 로 넣어 보고 되돌렸다 (2026-08-25 · 실측 전문은 TODO
     *       `ui-icon-blockify` 의 ⏩ '아바타 블록화 A/B' 블록). 같은 실험을 다시 하지 말 것.**
     *       ⓐ **위 '얼굴이 뭉갠다'는 40칸 override 에서는 안 일어난다** — 확대해 보면 왕관·송곳니·
     *          눈까지 그대로 읽힌다(그 문장은 **전역 20칸**을 전제로 쓴 것이라 지금도 맞다).
     *       ⓑ **얻는 것**: `probe-icon-keyline` 미달 **3종 → 0종**(153/3 → 156/0). 이 자의 마지막
     *          빨간 줄 셋이 전부 아바타였고, 원인은 '테가 없다'가 아니라 **테가 1칸(=4px)이라
     *          28px 표시에서 반 화소 아래로 뭉개지는 것**이다. 블록화의 경계 잉크가 그걸 한 칸으로 세운다.
     *       ⓒ 🚨 **잃는 것 — 순검정이 남색이 된다.** 이 24종의 팔레트는 원본 스크린샷에서 실측한
     *          것이고 키라인·머리가 `#000000` 이다. 그런데 `LFLOOR`(0.25)는 **속 칸을 순검정으로
     *          안 둔다** — 테는 잉크로 남고 **머리·상의의 속살만 남색으로 뜬다**(뱀파이어·좀비에서
     *          눈에 띈다). `cmp-avatar-style` 실측: 거의-검정 비율 **26.7% → 15.5%**(원본 35.7%) =
     *          근검정 화소의 42% 가 사라진다. 살빛 세로 높이도 **27.5% → 43.3%**(원본 24.6%)로 벌어진다.
     *       👉 **판단**: 비평가가 아바타를 지적한 문구는 *"결이 2~3배 곱다"*(=격자 해상도)인데,
     *          이 변경은 **격자를 안 건드린다**(넣으나 마나 40칸이다). 즉 **지적을 안 닫으면서
     *          원본 팔레트에서는 멀어진다.** 게이트는 비평가 채점이고 이 루틴은 그걸 못 돌리므로,
     *          **원본 실측 팔레트를 지키는 쪽(현행 유지)** 으로 두고 A/B 레시피만 남긴다.
     *          채점을 돌릴 수 있는 세션은 위 한 줄만 바꿔 두 판을 나란히 재 볼 것.
    ...
    // 어두운 면에 바닥을 **두 톤**으로 주는 이름인가(위 BLOCK.DEEP_FLOOR 주석 참고)
    _sceneArt(name) {
        for (const p of this.BLOCK.SCENE_ART) if (String(name).indexOf(p) === 0) return true;
        return false;
    },
    _blockSkip(name) {
    ...
    _blockify(src, W, H, cells, name) {
        ...
        /* 이 종이 **프레임을 가득 채우는 장면 그림**인가(던전 배너). 아이콘은 투명 바탕에 뜬
           실루엣이라 캔버스 테두리가 곧 실루엣 경계지만, 장면 그림에서 테두리는 **자른 자리**지
           물체의 경계가 아니다 — 거기에 키라인을 찍으면 원본에 없는 검은 액자가 둘린다. */
        const SCENE = this._sceneArt(name);
        const edge = (x, y) => (
            (!SCENE && (x === 0 || y === 0 || x === gx - 1 || y === gy - 1)) ||
            (x > 0 && !alive[y * gx + x - 1]) || (x < gx - 1 && !alive[y * gx + x + 1]) ||
            (y > 0 && !alive[(y - 1) * gx + x]) || (y < gy - 1 && !alive[(y + 1) * gx + x])
        );
        ...
        for (let y = 0; y < gy; y++) for (let x = 0; x < gx; x++) {
            ...
            if (l <= B.KEYL) {
                ...
                /* 속 칸은 검정이 아니라 '가장 어두운 톤'. 넓은 장면 일러스트(`SCENE_ART`)만
                   바닥을 둘로 쪼갠다 — 바닥 버킷(l===0)까지 내려온 칸은 반 칸 더 아래로.
                   바닥이 하나뿐이면 그 아래가 전부 한 색으로 모여 어두운 면의 층이 지워진다. */
                l = (SCENE && l <= 0) ? B.LFLOOR_DEEP : B.LFLOOR;
...
(function (G) {
    ...
    /* 💤 Zzz — **칸 정렬 비트맵으로 직접 찍는다.** (2026-08-25 종별 재작화)
       ⚠️ Z 를 셋 넣으면 23px 에서 서로 엉켜 지그재그 덩어리로 뭉갠다(1차 렌더) — 큰 Z 하나 +
       확실히 떨어뜨린 작은 Z 하나가 상한이다.
       🚨🚨 **경로(`plate` + 폴리곤)로 그리는 판은 폐기했다 — 이 항목의 규칙표가 이미 답을 갖고 있었다**
       (*"8칸 미만 모티프는 칸 정렬 비트맵으로 직접 찍을 것 — 경로로 그리면 무슨 수를 써도 얼룩"*).
       앞 세션이 사선 두께 배수(`dg = 1.6t`)로 두 번 손봤는데도 48px 실측에서 여전히
       **사선이 증발하고 카운터가 검게 메워져** 파란 덩어리 둘로 떴다(색 10개 · 갈색 잡티 26px).
       뿌리는 배수가 아니라 **경로+안쪽 키라인**이라는 조합 자체다:
       ⑴ `plate` 의 `lw` 키라인은 경로 위에 걸터앉아 **안쪽으로** 먹는데, Z 의 사선은 수직 두께가
          가로 두께의 0.71배라 20칸에서 가장 먼저 굶는다. ⑵ Z 의 카운터(위아래 삼각 홈)는 안쪽
          키라인 둘이 양쪽에서 좁혀 닫힌다. ⑶ 사선은 정의상 계단인데 경로 기울기가 칸에 안 떨어지면
          칸마다 다른 중간색이 나온다(`autoloop`·`ticket` 과 같은 병).
       → **글리프를 칸 지도로 찍고, 키라인은 그 지도를 한 칸 부풀린 _바깥_ 테두리로 만든다.**
          바깥에만 두르므로 가로획·사선·카운터가 설계한 칸 수를 **그대로** 갖는다.
       🚨 **`N` 은 `BLOCK.CELLS`(20)와 같아야 한다** — 다르면 칸 경계가 어긋나 다시 중간색이 생긴다.
          `CELLS` 를 바꾸는 세션은 이 함수도 같이 볼 것.
       📐 카운터 폭: 큰 Z 는 최대 7칸 · 작은 Z 는 4칸이고, 바깥 테두리가 각각 2칸을 먹어
          **5칸 / 2칸**이 남는다(규칙표의 '틈 최소 2칸' 하한을 작은 Z 가 정확히 만족한다 —
          작은 Z 를 더 줄이지 말 것). */
    G.draw.zzz = function (ctx, S) {
        const N = 20, U = S / N;                              // 블록화 격자와 **같은** 20칸
        /* 🚨 두 파랑은 **밴드 한가운데**를 겨냥해 역산한 값이다(피벗 실측 0.50 · `LCON` 1.2 →
           `l = 0.5 + (target − 0.5)/1.2`). 종전 `#a9d6f5`(L .812)·`#5f9ecf`(L .592)는 변환 뒤
           각각 **0.875·0.625 = 밴드 경계**에 앉아, 칸별 흔들기가 경계를 넘나들며 **한 색이 두 톤으로
           쪼개져** 가로획·사선이 얼룩덜룩했다(`shop_gems` 의 '쓰지도 않은 네 번째 톤'과 같은 병). */
        const K = '#17181a', LIGHT = '#8cb8dd', BASE = '#3878c7';
        const ink = new Map();                                // "cx,cy" → 색
        const put = (cx, cy, c) => { if (cx >= 0 && cy >= 0 && cx < N && cy < N) ink.set(cx + ',' + cy, c); };
        /* Z 한 글자를 칸 지도에 찍는다. w·h·t(가로획 두께)·d(사선 가로폭) 전부 **칸 단위**.
           사선은 행마다 시작 칸을 선형 보간해 찍으므로 45° 계단이 정확히 칸에 떨어진다. */
        const stamp = (ox, oy, w, h, t, d, top, bot) => {
            for (let r = 0; r < h; r++) {
                if (r < t || r >= h - t) {                    // 위·아래 가로획
                    for (let c = 0; c < w; c++) put(ox + c, oy + r, r < t ? top : bot);
                } else {                                      // 사선 — 오른위에서 왼아래로
                    const span = h - 2 * t - 1;
                    const xs = span > 0 ? Math.round((w - d) * (span - (r - t)) / span) : 0;
                    for (let c = 0; c < d; c++) put(ox + xs + c, oy + r, bot);
                }
            }
        };
        stamp(3, 2, 14, 16, 4, 4, LIGHT, BASE);               // Z 한 글자 — 크기 근거는 위 📐
        // 키라인 = 글리프를 한 칸 부풀린 테두리(8이웃). **바깥에만** 두르므로 속살·카운터를 안 먹는다.
        ctx.fillStyle = K;
        const edge = new Set();
        ...
        for (const [key, c] of ink) {
            const [cx, cy] = key.split(',').map(Number);
            ctx.fillStyle = c; ctx.fillRect(cx * U, cy * U, U, U);
        }
    };
...
(function (G) {
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        /* 달 — 원본 실측(중심 0.365H · 반경 0.19H). 중심을 **실측값 0.365H 로 맞췄다**
           (2026-08-25 `dg-band-luma`): 0.375H 는 달 아랫날을 밴드6(y 0.50~0.58H)까지 밀어 넣는데,
           그 밴드는 원본이 거의 검은 자리(루마 33.3 · 암부 88.6%)라 달 조각 하나가 그대로 초과분이
           된다(`tools/diag-dg-band.js` 실측: 밴드6 의 9.9% 를 루마 200 짜리 달빛이 차지). 반 칸
           올리는 것만으로 그 조각이 밴드5 로 넘어가고, 동시에 **원본 실측값에 더 가까워진다.** */
        const mx = W * 0.578, my = H * 0.365, mr = H * 0.175;
        ...
        (() => {
            /* R2 A2·B2 공통 '유령이 작고 회색이라 조연': 1.35배 확대 + 순백 + 묘비와 같은 근흑
               ...
               🚨 **이 유령은 표시 크기에서 가로 5칸 · 세로 6칸이다 — 어떤 디테일도 안 선다.**
                  (2026-08-25 `dg-band-luma` 에서 칸 정렬 비트맵으로 다시 찍어 보고 되돌린 자리.
                  네 판을 실측했으니 **다음 세션은 같은 넷을 다시 밟지 말 것**):
                    · 눈 1칸(얼굴 복판) → `_despeckle` 의 `MODE_MAJ`(3×3 최빈이 4칸 이상이고 더
                      밝으면 덮는다)가 **통째로 메운다**.
                    · 눈 2×2 → 얼굴이 5칸뿐이라 눈 둘이 4칸을 먹어 **얼굴이 T 자로 남는다**.
                    · 눈 1칸을 테에 붙임 → 살긴 사는데 좌우가 서로 다른 톤(L56·L112)으로 갈려
                      **눈이 아니라 잡티**로 읽힌다.
                    · 둥근 마루(윗줄 모서리 깎기)·자락 물결(아랫줄 한 칸 걸러 비우기) → **둘 다
                      메워져** 결국 직사각형으로 돌아온다.
                  👉 즉 **경로냐 비트맵이냐의 문제가 아니라 칸이 모자란 것**이고, 크게 그리는 것
                     말고 답이 없다. 그런데 **크게 그리면 값 게이트가 막는다** — 흰 줄 하나가
                     측정창의 4%p 라 `probe-dungeon-value` 밴드5(지금 +20.4)가 그 자리에서
                     ±22 를 넘는다. 이게 이 종에 남은 진짜 갈림길이다(TODO `dg-band-luma` 참조). */
            ...
            /* 자락 쪽 그늘 한 단(전경 소품 2톤 문법). 🔑 **반투명 → 불투명 실색으로 바꿨다
               (2026-08-25 `dg-band-luma`)**: `rgba(90,105,160,.28)` 는 순백 위에서 (208,213,228)
               = 흰색과 **같은 버킷**이라 블록화 뒤 톤이 안 갈렸다(실측 — 갈린 건 `_foldPalette` 가
               128/64 회색으로 접은 부산물뿐이라 '면'이 아니라 얼룩이었다). 유령이 몸통 전체를
               순백 한 덩어리로 들고 있었고, 그게 `probe-dungeon-value` 밴드6 초과(+31.6)의 절반이다
               (원본의 같은 자리는 언덕·묘비라 거의 검다).
               🚨 값은 **버킷 한가운데를 겨냥해 역산**한다(파일 규칙): 목표 l' 0.5 → 소스 l ≈ 0.44.
               🚨 경계 y 는 **칸 격자(H/20 = 0.05H)에 스냅**하되 자리는 조형이 아니라 **값이 정한다**:
                  0.55H(칸 11)여야 하고 0.60H(칸 12)로 내리면 밴드6 이 +30.1 로 게이트를 넘는다(실측). */
            ctx.save();
            /* 🚨 클립은 **몸통 + 팔 둘의 합집합**이다. 몸통만 클립하면 팔이 순백으로 남아 자락
               높이에 흰 조각 둘이 떠 '흰 뚜껑 달린 상자'로 읽힌다(실측 — 밴드6 초과의 잔여분도
               정확히 그 두 조각이었다). 획은 넓이가 0 이라 경로를 이어 붙여도 클립이 안 변한다. */
            ctx.beginPath(); body(); armL(); armR(); ctx.clip();
            ctx.fillStyle = '#54648c';                                                            // l .44 → 버킷 .5 (달빛 받은 자락의 그늘 면)
            ctx.fillRect(gx - r * 1.6, H * 0.55, r * 3.2, H * 0.35);
            ctx.restore();
        })();
        ...
        (() => {
            ...
            for (let i = 0; i < 14; i++)                                   // 잎 덩어리 · R9 2인 공통 '잎 없는 마른 나무로 읽힌다': 9→14 + 아래로 확장해 캐노피 덩어리를 키운다
                /* 병합 절충(두 스트림 비평가가 정반대 지적): 병렬 R '하늘과 값이 붙어 실루엣 소실 — 밝게'
                   ...
                   🔑 **`LFLOOR_DEEP` 이후 이 절충은 다시 풀렸다 (2026-08-25 `dg-band-luma`)**. 두
                   지적이 정면충돌한 이유는 **하늘 바닥이 하나뿐이라 캐노피가 내려갈 칸이 없었던 것**
                   이다 — 내리면 하늘(당시 단일 톤 L48)에 붙어 실루엣이 사라지니 밝게 둘 수밖에
                   없었고, 그러면 밤 장면에서 혼자 L79 로 떴다. 이제 하늘이 **L48/L24 두 톤**으로
                   갈렸으므로 캐노피를 한 칸 내려도 아래 톤(L24)과 16 만큼, 색상으로 한 번 더 갈린다.
                   🚨 두 색은 **일부러 서로 다른 버킷에 떨어뜨린다** — `#122a19` 는 바닥 버킷(→L40),
                   `#20452c` 는 그 위 버킷(→L79). 한 버킷에 몰면 캐노피가 다시 단색 덩어리가 된다
                   (그게 원래 지적이던 '잎 없는 마른 줄기'의 진짜 정체다: 어두워서가 아니라 면이
                   하나여서 잎으로 안 읽힌다). 값을 손보려면 반드시 `tools/diag-dg-band.js` 로
                   밴드별 상위 색을 다시 셀 것 — 두 색이 같은 버킷에 붙었는지는 눈으로 안 보인다. */
                blobs.push([x + H * (-0.26 + R() * 0.58), H * (-0.10 + R() * 0.48), H * (0.12 + R() * 0.085), i % 2 ? '#122a19' : '#20452c']);
