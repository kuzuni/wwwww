...
const IconGen = {
    ...
    BLOCK: {
        ...
        LFLOOR: 0.25,
        /* 🔀 **미채택 갈림길 — 어두운 면이 통째로 한 색이 되는 문제(2026-08-25 UI 스트림 실측).
         *    되풀이하기 전에 아래 실측 넷을 먼저 읽을 것.**
         *    `LFLOOR` 는 "순흑은 정보의 소실"이라 둔 바닥인데, **바닥이 하나뿐이면 그 아래 것이
         *    전부 거기로 모인다** — 지키려던 '내부 음영'을 바로 그 바닥이 지운다.
         *    ⑴ **증상(블록화 전 → 후, 같은 행 네 자리 루마)**:
         *       · `dg_invasion` 전경 **24·29·21·24 → 62·62·62·62**(한 색 80,56,48 로 평탄화)
         *       · `dg_ghost`   중경 **22·14·50·24 → 49·49·49·49**(한 색 32,48,96)
         *       원본은 21~50 의 폭을 가진 층인데 클론은 **단일값**이다.
         *    ⑵ **처방과 그 효과**: 바닥 버킷(0)까지 내려온 칸만 반 칸 더 아래(`0.125`)로 내리면
         *       어두운 면에 **두 톤**이 서고, `probe-dungeon-value` 초과 밴드가 **9/48 → 5/48**
         *       (침략 3/12 → **0/12**)로 줄었다. 그림도 확실히 좋아진다.
         *    ⑶ 🚨 **그런데 `probe-emblem-core` 가 깨진다** — 미달 0종 → **3종**
         *       (`sk_powerStrike` 11.8% · `sk_whirlwind` 29.4% · `sk_pierceShot` 29.8%, 게이트 34%).
         *       그 자는 '속살 = 루마 ≥ 50' 을 세는데, **문장들은 `LFLOOR`(0.25)가 어두운 속살을
         *       루마 50 바로 위로 들어올려 준 덕에 통과하고 있었다.** 즉 그 통과는 조형이 아니라
         *       바닥 보정이 만든 것이다.
         *    ⑷ 🚨 **배너의 '어두운 면'과 문장의 '어두운 속살'을 가를 축을 셋 재 봤는데 전부 실패했다**
         *       (다음 세션이 같은 셋을 다시 재지 말 것):
         *       | 축 | 배너 | 깨진 문장 | 판정 |
         *       |---|---|---|---|
         *       | 바닥 버킷 칸 비율 | 침공 42.6% · 유령 48.6% | 회오리 41.8% · 관통 43.7% | **겹침** |
         *       | 피벗(평균 명도) | 유령 0.17 | 강타 0.17 | **동일** |
         *       | 최대 연결 덩어리 | 침공 37.3% · 유령 45.1% | 회오리 11.3% · 관통 13.6% · **강타 33.9%** | 강타가 침공과 겹침 |
         *       → 즉 **자동 판별축이 없다.** 남은 길은 ⓐ `probe-emblem-core` 게이트를 다시 세우거나
         *       (지금 값이 바닥 보정에 기대고 있다는 것을 위에서 확인했다) ⓑ `dg_*` 같은 **넓은 장면
         *       일러스트에만** 적용하는 것(가로 3.45:1 = `dg_` 4종뿐이라 사실상 종별 특례)이다.
         *       ⓑ 는 4종만 위한 특례라 이 세션은 안 골랐다 — **사용자/다음 세션이 정할 자리다.**
         *    ⚠️ 값을 0 으로 두는 길은 없다. 그러면 `LFLOOR` 를 없앤 것과 같아져 라운드1 지적
         *       ("벡터 실루엣" · "하이라이트 칸이 하나도 없다")이 그대로 돌아온다. */
        /* 🚨 **`LFLOOR` 의 대칭 상한 — 채도가 있는 칸은 순백까지 올리지 않는다.**
         *    `LFLOOR` 는 "순흑은 다크 엔드가 아니라 정보의 소실"이라 두었는데, **밝은 쪽에는
         *    같은 가드가 없었다**(2026-08-25 UI 스트림에서 발견). 그런데 HSL 에서 `l = 1` 은
         *    색상·채도를 **통째로 버린다** — `_hsl2rgb(h, s, 1)` 은 h·s 가 무엇이든 순백이다.
         *    즉 위쪽 버킷은 아래쪽보다 더 파괴적이다(순검정은 실루엣이라도 남지만 순백은
         *    배경과도 안 갈린다).
         *    실측(던전 배너, 블록화 전→후 하늘색):
         *      · `dg_hammer`   170,221,255(하늘색)  → **255,255,255** · 흰 화소 0% → 23.6%
         *      · `dg_invasion` 254,233,203(노을)    → **255,255,255** · 0% → 14.6%
         *      · `dg_zombie`   204,177,255(연보라)  → **255,255,255** · 0% → **40.7%**
         *      · `dg_ghost`    27,35,68(밤하늘)     → 32,48,96(유지)  · 0% →  2.7%
         *    밤 배너만 멀쩡했던 이유가 이것이다 — 어두운 하늘은 위 버킷에 안 닿는다.
         *    비평가가 배너를 두고 "혼자 튄다"·"반투명 보라 안개"라 한 자리가 여기다.
         * 🚨 **문턱은 HSL 채도가 아니라 크로마(max−min)로 재야 한다 — 실측으로 갈렸다.**
         *    처음엔 `s > 0.15` 로 걸었는데 유령·주사위·도끼날 같은 **진짜 흰 것까지** 끌어내려
         *    연보라 유령·회색 주사위가 됐다. 이유: **HSL 채도는 근백색에서 변별력이 없다** —
         *    유령의 흰 몸(242,245,255)도 하늘색(170,221,255)도 둘 다 `s = 1.0` 으로 잰다
         *    (L>0.5 에서 `s = (max−min)/(2−max−min)` 라 max 가 255 에 붙으면 분모도 같이 0 으로 간다).
         *    반면 **크로마는 깨끗이 갈린다**: 유령 0.051 · 주사위 0.05 대 하늘 0.333(해머)
         *    · 0.306(좀비) · 0.200(침공). 그래서 0.12 를 문턱으로 둔다.
         * ⚠️ 크로마는 **칸 원색**(스냅 전)에서 잰다 — SLIFT 로 부풀린 뒤에 재면 옅은 색조가
         *    다시 문턱을 넘어 같은 함정에 빠진다.
         * 🚨 **값 0.875 는 격자(LSTEP 5 = {0,.25,.5,.75,1}) 밖의 반 칸이다 — 일부러 그렇다.**
         *    처음엔 '바로 아래 버킷'인 0.75 를 썼는데, 그러면 순백(너무 밝음)에서 한 칸을 통째로
         *    건너뛰어 **이번엔 너무 어두워진다**. `probe-dungeon-value`(원본 배너와 밴드별 휘도 대조)
         *    로 잰 실측:
         *      · HEAD(상한 없음) 초과 11/48 — 좀비 **+45.8**(순백이라 원본보다 밝다)
         *      · LCEIL 0.75      초과 **18/48** — 좀비 −42.7 (반대편으로 넘어가 악화)
         *      · LCEIL 0.8125    초과 11/48
         *      · **LCEIL 0.875   초과 9/48** — 좀비 초과밴드 3/12 → **0/12**
         *    즉 이 상한은 선재 FAIL(11/48)을 **악화가 아니라 개선**시키는 자리에 있어야 한다.
         *    반 칸이 팔레트를 늘리지도 않는다 — 오히려 순백이 한 색 빠져 **평균 11.1 → 8.1색**
         *    으로 줄었다(`probe-icon-blockify` 실측). 되돌리기 전에 저 네 줄을 다시 재 볼 것. */
        LCEIL: 0.875,
        LCEIL_C: 0.12,
    ...
    _blockify(src, W, H, cells, name) {
        ...
        for (let y = 0; y < gy; y++) for (let x = 0; x < gx; x++) {
            ...
            // 칸 원색의 크로마 — 위 LCEIL 주석 참조(HSL 채도로는 근백색을 못 가른다).
            ...
            } else if (l >= 1 && chroma > B.LCEIL_C) {
                l = B.LCEIL;                       // 채도가 있는 칸은 '가장 밝은 톤'까지 (순백은 색상을 버린다)
            }
    ...
    draw: {
        ...
        /* ⚔ 중세 시대 — 검. (2026-08-25 종별 재작화 — `age_interstellar` 과 **같은 지적 ⓑ** 를 공유한다)
           라운드1·3 비평가 지적: *"회색→흰색이 길게 매끈하게 이어지는 램프. 마크 금속은 2~3톤으로 끊는다."*
           뿌리는 칼날이 `_ageStroke` 의 **2스톱 그라디언트를 날 길이 방향으로** 깔고 있던 것이다 —
           블록화는 색을 칸에 다질 뿐 **어느 방향으로 흐르는지는 못 고친다.**
           → 칼날을 **평면 두 톤의 45° 띠**로 다시 그린다: 왼위 절반이 밝은 면(칼날 서슬), 오른아래
             절반이 본색. 톤 경계는 날과 나란한 45° 라 칸 계단이 날 방향과 어긋나지 않는다.
           🚨 **날 각도를 정확히 45° 로 둘 것.** 종전은 (0.36,0.66)→(0.84,0.20) 이라 Δx 0.48 · Δy 0.46
              으로 45° 에서 살짝 어긋나 있었고, 그러면 계단이 **몇 칸마다 한 번씩 어긋나** 눈에는
              안티에일리어싱으로 되돌아온다(`ticket` 의 −0.14rad 회전과 같은 병).
           🚨 **칼끝은 점이 아니라 90° 모서리다** — 45° 띠의 두 변은 평행이라 절대 한 점에서 안 만난다.
              가로 한 변 + 세로 한 변으로 각진 모서리를 만드는 게 픽셀아트 검의 정석이고, 마크 검도 그렇다.
           📌 손잡이·가드는 지적에 없어 조형을 그대로 뒀고, **그라디언트만 평면색으로** 바꿨다
              (같은 램프가 작게나마 남아 있었다). */
        age_medieval(ctx, S) {
            const G = IconGen;
            /* 🚨 **본색은 밝은 강철이 아니라 _어두운_ 강철이다 — 밝게 뒀다가 되돌렸다.**
               `#98a3ae`(변환 뒤 0.75 밴드)로 두니 48px 출력에 그 색이 **3칸**밖에 안 남았다:
               본색 띠가 1.41칸인데 바로 옆 키라인도 1.41칸이라, 대각선에서 반쯤 걸친 칸을 최빈이
               전부 어두운 쪽으로 몰아 `LFLOOR`(0.25)로 내려앉힌다. 즉 **화면은 이미 흰 서슬 + 어두운
               그늘 두 톤**인데 소스만 세 톤인 척하고 있었다. 그래서 본색을 0.25 밴드로 내려 잡아
               코드와 화면을 일치시켰다(이 아이콘의 피벗은 실측 0.5 — 가드 .549→.5 · 손잡이 .375→.25). */
            const K = '#17181a', LIT = '#eef2f6', BODY = '#4c5661';
            const path = (pts) => {
                ctx.beginPath();
                pts.forEach((p, i) => (i ? ctx.lineTo(S * p[0], S * p[1]) : ctx.moveTo(S * p[0], S * p[1])));
                ctx.closePath();
            };
            // 손잡이·가드도 **정확히 45°** 로 맞췄다 — 종전은 Δx≠Δy 라 계단이 몇 칸마다 어긋났다.
            // 🚨 가드는 날의 **양옆으로 다 나와야** 가드로 읽힌다 — 짧게 두면 날 밑동에 먹혀
            //    손잡이와 한 덩어리 주황 얼룩이 된다(실측). 날 밑동(0.30,0.60)~(0.50,0.80) 보다 길게 잡는다.
            G._ageStroke(ctx, S, [[0.14, 0.90], [0.32, 0.72]], 0.13, [[0, '#a5651a'], [1, '#a5651a']]);   // 손잡이
            G._ageStroke(ctx, S, [[0.22, 0.56], [0.52, 0.86]], 0.09, [[0, '#d99a3f'], [1, '#d99a3f']]);   // 가드
            // 칼날 — 검정 실루엣 → 한 칸 들여 본색 → 왼위 절반에 밝은 면(본색 다각형으로 클립)
            /* 🚨 날 폭은 **수직 5.66칸**(테 1.41 + 밝은 면 1.41 + 본색 1.41 + 테 1.41)이다 — 늘리지 말 것.
               한 판 더 넓게(7.07칸) 그려 봤더니 손잡이가 상대적으로 작아져 **검이 아니라 식칼**로 읽혔다.
               45° 띠의 폭은 두 평행선의 절편 차 ÷ √2 다: 검정 0.90↔1.30 · 속살 1.00↔1.20. */
            path([[0.30, 0.60], [0.80, 0.10], [0.95, 0.10], [0.95, 0.35], [0.50, 0.80]]);
            ctx.fillStyle = K; ctx.fill();
            const core = [[0.40, 0.60], [0.85, 0.15], [0.90, 0.15], [0.90, 0.30], [0.50, 0.70]];
            path(core); ctx.fillStyle = BODY; ctx.fill();
            ctx.save();
            path(core); ctx.clip();
            // 날과 나란한 45° 띠 — 양 끝을 캔버스 밖까지 늘려 클립이 끝을 잘라 준다
            path([[0.20, 0.80], [1.05, -0.05], [1.15, -0.05], [0.30, 0.80]]);
        ...
        /* 🔫 성간 시대 — 광선총. (2026-08-25 종별 재작화)
           종전은 45° 로 기운 `_ageStroke` 두 개(2스톱 그라디언트) + `ctx.arc` 발광부였다. 결함 셋:
           ⑴ **비평가 1·3라운드 공통 지적 "회색→흰색이 길게 매끈하게 이어지는 램프"** — 그라디언트를
              총열 방향으로 깔아 놨으니 블록화 뒤에도 톤이 길이를 따라 흘렀다(마크 금속은 2~3톤으로 끊는다).
           ⑵ **`ctx.arc` 참된 원호** — 이 항목이 확립한 규칙("블록 화법에서 경로는 수평·수직·45° 만")을
              정면으로 어긴다. 칸마다 다른 각도로 스쳐 경계 칸이 전부 다른 회색이 되고, 눈에는 그게
              안티에일리어싱으로 되돌아온다(`autoloop` 과 같은 병).
           ⑶ 🚨 **`age_medieval`(검) 과 실루엣이 거의 같았다** — 둘 다 45° 막대 하나라 48px 에서
              구별이 안 됐다. 시대 아이콘은 한 화면(제련 확률표)에 10종이 세로로 붙어 뜨므로
              이건 종 판독 결함이다.
           → **칸 정렬 비트맵**으로 다시 그린다: 손잡이·몸통·발광부를 각각 격자에 딱 맞는 상자로 두고,
             키라인은 스트로크가 아니라 **한 칸 안쪽 들여찍기**로 낸다(스트로크는 경로 위에 걸터앉아
             반 칸씩 먹는다 — 4칸짜리 부품에서 색이 한 줄도 안 남는다). 몸통은 가로 3톤 띠.
           🚨 **좌표를 0.05 배수에서 떼지 말 것** — 그 순간 경계가 두 칸에 걸쳐 평균색이 다른 단계로
              스냅돼 면 사이에 한 칸짜리 톱니가 생긴다(이 항목의 규칙표 참조). */
        age_interstellar(ctx, S) {
            const K = '#17181a';
            const LIT = '#eef2f6', MID = '#9aa4ae', DK = '#5c666f';
            // 발광부 — 양자(시안)·다중우주(분홍)와 안 겹치는 보라. 두 톤은 변환 뒤 밴드 한가운데
            // (0.75 · 0.50)에 앉게 역산한 값이다 — 경계에 두면 흔들기가 한 색을 두 톤으로 쪼갠다.
            const CORE = '#b06cf0', CORE_DK = '#7d29d6';
            const box = (x0, y0, x1, y1, fill) => { ctx.fillStyle = fill; ctx.fillRect(S * x0, S * y0, S * (x1 - x0), S * (y1 - y0)); };
            // ① 손잡이(먼저) — 몸통이 위를 덮어 프레임처럼 물린다. 총열 뒤쪽에 둬야 권총으로 읽힌다
            box(0.15, 0.50, 0.35, 0.90, K);
            box(0.20, 0.55, 0.30, 0.85, DK);
            // ② 총열 — 키라인 한 칸 + 속살 가로 3톤(위 밝은 면 · 본색 · 아래 그늘)
            box(0.10, 0.30, 0.70, 0.55, K);
            box(0.15, 0.35, 0.65, 0.40, LIT);
            box(0.15, 0.40, 0.65, 0.45, MID);
            box(0.15, 0.45, 0.65, 0.50, DK);
            // ③ 발광부 — 총열보다 **위아래로 크게** 뽑아 머리가 서게 한다(같은 높이면 그냥 상자 두 개다).
            //    총열과 맞붙여 키라인 두 칸이 사이를 가른다(1칸 틈은 MODE_MAJ 가 메운다)
            box(0.70, 0.20, 0.95, 0.65, K);
            box(0.75, 0.25, 0.90, 0.55, CORE);
            box(0.75, 0.55, 0.90, 0.60, CORE_DK);
        },
        ...
        /* 🕊 신성 시대 — 황금 날개 한 쌍. (2026-08-25 종별 재작화)
           종전은 반쪽마다 8각 폴리곤 **하나**였고, 깃 홈을 `[0.84,0.42] → [0.96,0.40]` 같은
           **세로 0.02(=0.4칸)** 짜리 되꺾임으로 냈다. 이 항목의 규칙표가 정한 '디테일 최소 1칸'에
           한참 못 미쳐 20칸 다운샘플에서 홈이 통째로 사라졌고, 거기에 `_despeckle` 의 `MODE_MAJ`
           (3×3 최빈이 제 색보다 밝으면 덮는 규칙)가 남은 1칸 틈까지 메워 **노란 덩어리 두 개**가 됐다.
           → 인계 처방("깃을 3~4개로 줄이고 홈을 2칸으로 벌릴 것")대로 **깃을 3장으로 줄인다.**
             다만 홈은 처방의 '2칸 빈틈'이 아니라 **겹친 키라인 한 칸**으로 낸다(아래 🚨🚨 가 그 이유).
             깃마다 세로 6칸이라 키라인 한 칸씩을 위아래로 빼고도 **색 네 줄**(밝은 면 + 본색 3줄)이 남는다.
           🚨 **키라인을 `_agePoly` 의 스트로크로 두르지 않는다** — 스트로크는 경로 위에 걸터앉아
              안쪽으로 반 칸을 먹는다. 4칸짜리 깃에서는 위아래 **두 칸씩**이 검게 스냅돼 색이 한 줄도
              안 남는다(실측). 대신 검정 상자를 깔고 **한 칸 들여** 색을 얹는다 = 두께가 칸에 딱 맞는 테.
           🚨🚨 **홈을 '빈칸'으로 내지 말 것 — 두 판을 태우고 얻은 결론이다(음성 결과 2건).**
              ⑴ 깃 3장을 투명한 2칸 틈으로 떼어 놓으니 각 깃에 사방 키라인이 둘려 **검은 테를 두른
                 타일 6장**이 됐다(48px 실측: 검정 488px 대 금색 208px = 화소의 7할이 테).
              ⑵ 그래서 안쪽 등뼈로 이어 봤더니 이번엔 깃이 등뼈에 수직으로 붙은 **대문자 'F' 두 개**로
                 읽혔다(등뼈가 4칸이라 기둥처럼 서고, 깃 길이 차가 작아 빗처럼 보인다).
              → **깃끼리 한 칸 겹쳐 쌓는다.** 아래 깃이 위 깃의 아래 키라인을 덮어써 홈이 **정확히
                한 칸**으로 선다(마크가 깃·잎을 가르는 바로 그 화법). 빈칸이 아니라 테가 홈이 되므로
                `_despeckle`/`MODE_MAJ` 가 메울 1칸 틈 자체가 없다.
              🚨 **겹치지 않고 맞대기만 하면 홈이 두 칸이 된다 — 그것도 태워 봤다.** 두 키라인이
                나란히 서서 홈이 색 줄만큼 두꺼워지고, 48px 실측에서 검정+회색이 654px 대 금색 378px
                로 다시 테가 그림을 이겼다. 겹침 한 칸이 그 차이를 만든다.
           🚨 **바깥 끝 계단(0.75 → 0.85 → 0.95)을 줄이지 말 것** — 이 테이퍼가 실루엣을 삼각형으로
              만들어 '날개'로 읽히게 한다. 길이가 비슷해지면 곧장 위 ⑵ 의 빗으로 되돌아간다.
           🚨 **안쪽 끝을 더 벌리지 말 것** — 두 날개의 중앙 틈이 정확히 2칸(0.45↔0.55)이다. */
        age_divine(ctx, S) {
            const K = '#17181a';
            /* 명도는 변환 뒤 밴드 한가운데(0.75 · 0.50)에 앉게 역산한 값이다.
               이 아이콘의 피벗(자기 평균 명도)은 실측 0.65 — LIT .708→.72 · BASE .525→.50.
               🚨 **세 번째(그늘) 톤은 일부러 안 쓴다 — 넣어 보고 뺐다.** 깃 사이 홈 칸은 위아래가
                  다 불투명이라 **경계 칸이 아니고**, 그래서 순검정 키라인이 아니라 `LFLOOR`(0.25)로
                  내려앉는다(실측 rgb(64,64,64)). 즉 홈이 이미 0.25 톤이다 — 거기에 0.25 짜리 어두운
                  금색 줄을 덧대면 **어두운 줄이 두 칸으로 붙어** 홈이 색 줄만큼 두꺼워진다. */
            const LIT = '#ffd86a', BASE = '#f0a81c';
            // 깃 하나 = 격자에 맞춘 상자(세로 6칸) + 바깥 끝 45° 모따기(=깃 끝).
            //   테는 스트로크가 아니라 **한 칸 들여찍기** — 스트로크는 경로에 걸터앉아 반 칸씩 먹는다.
            const wing = (dir) => {
                const X = (v) => 0.5 + dir * (v - 0.5);       // dir=-1 이면 좌우 반전
                // 바깥 끝을 위·아래로 모따기해 **깃 끝을 뾰족하게** 한다 — 상자로 두면 이 계단이
                // 날개가 아니라 '계단식 상자 더미'로 읽힌다(실측). 위 1칸 · 아래 2칸의 비대칭이
                // 깃 끝이 **아래-바깥**을 가리키게 만든다(대칭으로 두면 그냥 육각형이다).
                const poly = (x0, x1, y0, y1, cut, fill) => {
                    const pts = [[x0, y0], [x1 - cut / 2, y0], [x1, y0 + cut / 2], [x1, y1 - cut], [x1 - cut, y1], [x0, y1]];
                    ctx.beginPath();
                    pts.forEach((p, i) => (i ? ctx.lineTo(S * X(p[0]), S * p[1]) : ctx.moveTo(S * X(p[0]), S * p[1])));
                    ctx.closePath(); ctx.fillStyle = fill; ctx.fill();
                };
                const band = (x0, x1, y0, fill) => {
                    const a = X(x0), b = X(x1);
                    ctx.fillStyle = fill;
                    ctx.fillRect(S * Math.min(a, b), S * y0, S * Math.abs(b - a), S * 0.05);
                };
                /* [바깥 끝 x, 위 y] — 피치 5칸 · 높이 6칸 = 한 칸 겹침.
                   🚨 **아래로 갈수록 길어진다(0.75 → 0.85 → 0.95). 뒤집지 말 것** — 반대로
                      (위가 가장 긴) 배열도 그려 봤는데 위가 무거워 **버섯/열쇠**로 읽혔다.
                      접은 새 날개는 어깨가 위·긴 첫째날개깃이 아래-바깥이라, 이 방향이 실루엣을
                      '아래-바깥으로 뻗는 날개'로 만든다. */
                const F = [
                    [0.75, 0.05],                             // 위 깃 — 어깨(가장 짧다)
                    [0.85, 0.30],
                    [0.95, 0.55],                             // 아래 깃 — 날개 끝(가장 길다)
                ];
                for (const [x1, y0] of F) {                   // 검정 → 속살 순서로 한 깃씩 — 아래 깃이 위 깃의 아래 테를 덮는다
                    poly(0.55, x1, y0, y0 + 0.30, 0.10, K);
                    poly(0.60, x1 - 0.05, y0 + 0.05, y0 + 0.25, 0.05, BASE);
                    band(0.60, x1 - 0.10, y0 + 0.05, LIT);    // 밝은 면 = 깃 위 한 칸(모따기만큼 짧다)
                }
            };
            wing(-1); wing(1);
        },
...
(function (G) {
    ...
    /* 낱개 보석 — **3면 컷 젬**(좌상 밝은 면 · 우상 본색 면 · 아래 그늘 면).
     *
     * 🚨 **종전 조형(마름모 안에 작은 마름모 + 삼각 스펙큘러)은 48px 에서 분홍 점 잡티로 읽혔다**
     *    (블라인드 비평가 6인이 라운드4·5·7 에 걸쳐 `shop_gems2/3/4` 를 반복 지목:
     *     "단일 슬래브 + 분홍 점 노이즈"). 원인은 색이 아니라 **크기 대비 면 수**다:
     *     r=0.08 짜리 알은 48px 표시에서 폭 **3.2칸**인데 거기에 겉 마름모·속 마름모·스펙큘러
     *     세 겹을 넣으면 한 겹이 **한 칸도 못 차지해** 칸 스냅이 임의로 하나를 골라 버린다.
     *     후처리 상수로는 절대 안 고쳐진다(앞 세션이 `MODE_MAJ` 문턱 4까지 내려 확인) —
     *     **면을 그림에서 나눠 줘야** 한다.
     * 처방 두 개를 같이 건다:
     *   ⑴ **면을 명시적으로 셋으로 쪼갠다** — 허리선(가장 넓은 y) 위를 좌/우로 갈라 밝은 면·본색 면,
     *      아래 뾰족한 몫을 통째로 그늘 면. 명도차를 `LSTEP`(5단계 = 폭 0.2) 보다 크게 벌려
     *      칸 스냅 뒤에도 세 톤이 **반드시** 다른 단계에 떨어지게 한다.
     *   ⑵ **알을 키우고 수를 줄인다** — 한 면이 최소 1칸을 갖게 하려면 알 폭이 ≥5칸(r ≥ 0.12)이어야
     *      한다(48px·CELLS 20 → 칸 2.4px). 그래서 ②③④의 알 수를 6~9개에서 3~5개로 줄였다.
     *      '양이 늘수록 커진다'는 단계감은 알 개수가 아니라 **그릇 크기**가 진다(원래 그 설계다).
     * ⚠️ 스펙큘러는 **한 칸짜리 사각**으로 둔다 — 삼각으로 두면 칸 스냅에서 모서리가 반칸씩 깎여
     *    '분홍 점'으로 되돌아간다. */
    /* 좌상 밝은 면. 🚨 **밴드 경계에 걸터앉지 않게 명도를 역산해 잡는다(2026-08-25 실측).**
     * 종전 `#ff8fae`(L .78)는 변환 후 l' ≈ .875 = **.75 와 1.0 밴드의 정확한 경계**라, 칸별 흔들기가
     * 그 경계를 넘나들며 **한 색이 두 칸값으로 갈렸다**: 실측 팔레트에 `255,128,159`(704px)와
     * **순백 `255,255,255`(832px)** 이 나란히 떴다. 블라인드 비평가 A 가 "필요 없는 네 번째 톤"으로
     * 집은 게 바로 이것이다 — 스펙큘러가 아니라 **밝은 면 자신이 쪼개진 것**이었다.
     * → 목표 l' .72(밴드 [.625,.875] 한가운데)로 역산: `l = piv + (.72 − piv)/LCON`, piv ≈ .31~.40
     *   구간에서 L ≈ .66. 👉 **새 톤을 넣을 때는 눈이 아니라 이 역산으로 잡을 것.** */
    const GEM_LIT = 'hsl(345,100%,66%)';
    const GEM_DK = '#8e0f28';   // 아래 그늘 면 — RED 보다 명도 −0.22
    const gemDot = (ctx, S, x, y, r) => {
        const rx = r * 0.92, ry = r * 1.02;
        const T = [x, y - ry], R = [x + rx, y], B = [x, y + ry], L = [x - rx, y], C = [x, y];
        // ⚠️ 키라인 두께는 **보석 반지름에 비례**해야 한다 — 고정 두께로 두면 낱개가 작은 ②③에서
        // 검정이 알을 통째로 먹어 '어두운 덩어리'로 읽힌다(1차 렌더에서 실제로 그랬다).
        ink(ctx, S, poly(ctx, S, [T, R, B, L]), RED, r * 0.30);
        on(ctx, poly(ctx, S, [T, C, L]), GEM_LIT);   // ⑴ 좌상 밝은 면
        on(ctx, poly(ctx, S, [L, C, R, B]), GEM_DK); // ⑶ 아래 그늘 면 (우상 면은 RED 실루엣 그대로)
        /* 🚨 **순백 스펙큘러는 뺐다 (2026-08-25 라운드8 지적 + 최빈색 전환 실측).**
         * ⑴ 블라인드 비평가 A: "창백한 하이라이트는 이 재질에 **필요 없는 네 번째 톤**" —
         *    마크 보석은 키라인 + 3톤으로 끝낸다.
         * ⑵ 그리고 칸 색이 평균 → **최빈**으로 바뀐 뒤(같은 날 병렬 세션 커밋 19a52d8) 더 나빠졌다:
         *    반 칸짜리 흰 사각이 그 칸의 최빈이 되면 **칸 하나를 통째로 순백으로** 먹어, 작은 알에서
         *    흰 덩어리가 알의 1/4을 차지했다. 평균 시절엔 이웃과 섞여 분홍 한 칸으로 앉던 코드다.
         *    👉 **같은 코드가 파이프라인 변경으로 의미가 뒤집힌 사례** — `_blockify` 를 건드린 세션은
         *    반 칸 이하 디테일을 가진 종을 반드시 다시 볼 것(눈검사 없이는 안 보인다).
         * 밝은 면(`GEM_LIT`)이 이미 하이라이트 노릇을 하므로 광은 안 죽는다. */
    };
    const ground = (ctx, S, x, y, rx, ry) => on(ctx, ell(ctx, S, x, y, rx, ry), 'rgba(0,0,0,.13)');

    /* 그릇(자루·항아리·궤짝) 공용 3면 나무.
     * 🚨 **종전에는 `WOOD` 단색 + `rgba(0,0,0,.22)` 음영 오버레이였고, 48px 에서 통째로
     *    '단일 슬래브'로 읽혔다**(비평가 6인 반복 지목). 원인: 알파 음영은 칸 평균에 섞여
     *    `#79342d` → `#5e2823` 정도로만 내려앉는데, 그 둘은 `LSTEP` 5단계에서 **같은 단계**로
     *    스냅돼 화면에서는 차이가 0 이 된다. 알파를 걷어내고 **불투명 3톤**으로 바꾼다 —
     *    명도차를 단계 폭(0.2)보다 크게 벌려야 칸 스냅을 건너서 살아남는다.
     *    (마크 나무 블록도 늘 '윗면 밝음 · 정면 본색 · 옆면 그늘' 3톤이다.) */
    /* 🔬 **톤은 눈으로 고르면 안 된다 — 칸 스냅의 격자에 맞춰 고른다(이번 세션 실측).**
     * `_blockify` 는 명도를 `round(l·4)/4` = **{0, .25, .5, .75, 1}** 다섯 칸에만 떨어뜨리고,
     * 그중 `.25` 이하는 통째로 `LFLOOR(.25)`(속 칸) 또는 순검정(경계 칸)이 된다.
     * 종전 `WOOD #79342d`(L .325) → 피벗 .35 기준 확장 후 **.25** · `WOOD_DK #4d1f1a`(L .21) →
     * **.25** — 즉 **본색과 그늘이 같은 칸값으로 뭉개졌다.** 알파 음영(`rgba(0,0,0,.22)`)도
     * 마찬가지로 .25 안에서만 움직여 화면에서는 차이가 0 이었다. 이게 '단일 슬래브'의 정체다.
     * → 세 톤을 **격자 칸 세 개(.25 / .5 / .75)의 한가운데**를 겨냥해 다시 잡는다(피벗 ≈ .35 기준
     *   역산: l = piv + (target − piv)/LCON). 색상(hue)은 원본 계열(≈8°)을 유지한다.
     * ⚠️ 눈으로 '조금 밝게'는 소용없다 — 같은 칸 안에서 움직이면 화면에선 그대로다. */
    const WOOD_MID = '#ab5f48';  // 정면 본색  L .476 → 칸 .50
    const WOOD_LIT = '#dba286';  // 빛 면      L .692 → 칸 .75
    const WOOD_SH = '#6b3229';  // 그늘 면    L .290 → 칸 .25
    /* 🔬 **면 경계는 칸 격자(1/CELLS = .05)에 스냅한다.** 안 맞추면 경계가 두 칸에 걸쳐
     * 다운샘플이 두 톤을 평균 내고, 그 중간값이 다시 다른 단계로 스냅돼 **면 사이에 한 칸짜리
     * 톱니 줄**이 생긴다(실측: 항아리 좌/중 경계가 .387 이라 48px 에서 이가 빠져 보였다).
     * 마크 텍스처가 면 경계에서 깨끗한 이유가 이것이다 — 경계가 항상 칸 선 위에 있다. */
    const Q = (v) => Math.round(v * G.BLOCK.CELLS) / G.BLOCK.CELLS;

    // ① 60젬 — 낱개 3개(아래 2 · 위 1). 알을 키워 3면이 각각 칸을 갖게 한 대가로 개수를 줄였다.
    G.draw.shop_gems1 = function (ctx, S) {
        ground(ctx, S, 0.50, 0.845, 0.34, 0.070);
        const r = 0.205;
        gemDot(ctx, S, 0.290, 0.660, r);
        gemDot(ctx, S, 0.710, 0.660, r);
        gemDot(ctx, S, 0.500, 0.330, r);
    };

    // ② 220젬 — 보석이 목까지 차 넘치는 자루. 몸통을 좌(빛)·중(본색)·우(그늘) 세 세로 면으로 쪼갠다.
    G.draw.shop_gems2 = function (ctx, S) {
        ground(ctx, S, 0.50, 0.910, 0.30, 0.055);
        const neck = () => rrect(ctx, S, Q(0.35), Q(0.40), Q(0.30), Q(0.15), 0.045)();
        const body = () => ell(ctx, S, 0.500, Q(0.70), Q(0.25), Q(0.20))();
        ink(ctx, S, neck, WOOD_MID, 0.055);
        ink(ctx, S, body, WOOD_MID, 0.060);
        /* 면은 **폭을 3등분**해 통째로 깐다(경계는 칸 선 위). 종전처럼 좁은 띠로 두면 경계 칸이
         * 키라인에 먹혀 화면에 한 칸도 안 남는다(실측: 좌 빛 면이 48px 에서 1칸짜리 부스러기로 떴다). */
        ctx.save(); ctx.beginPath(); body(); ctx.clip();
        on(ctx, rrect(ctx, S, Q(0.25), 0.45, Q(0.15), 0.50, 0), WOOD_LIT);   // 좌 빛 면 (3칸)
        on(ctx, rrect(ctx, S, Q(0.60), 0.45, Q(0.15), 0.50, 0), WOOD_SH);    // 우 그늘 면 (3칸)
        ctx.restore();
        ctx.save(); ctx.beginPath(); neck(); ctx.clip();
        on(ctx, rrect(ctx, S, Q(0.35), 0.39, Q(0.10), 0.18, 0), WOOD_LIT);
        on(ctx, rrect(ctx, S, Q(0.55), 0.39, Q(0.10), 0.18, 0), WOOD_SH);
        ctx.restore();
        /* 목 위로 솟은 보석 2개.
         * 🚨 **알 사이는 최소 한 칸(=.05) 띄운다** — 겹치면 각자의 그늘 면(GEM_DK)이 이어 붙어
         *    48px 에서 다시 '어두운 한 덩어리 + 분홍 점'이 된다(고치려던 그 결함으로 되돌아간다).
         *    `gemDot` 의 반폭은 `0.92·r` 이므로 중심 간격 ≥ 0.92(r₁+r₂) + .05 이어야 한다. */
        gemDot(ctx, S, 0.395, 0.355, 0.125);
        gemDot(ctx, S, 0.625, 0.315, 0.125);
        // 앞으로 굴러 나온 보석 1개
        gemDot(ctx, S, 0.165, 0.820, 0.140);
    };

    // ③ 800젬 — 테두리 두른 큰 항아리. ②와 같은 3면 규칙 + 아가리 테가 한 단 더 밝아 '테'로 읽힌다.
    G.draw.shop_gems3 = function (ctx, S) {
        ground(ctx, S, 0.50, 0.935, 0.38, 0.050);
        const body = () => ell(ctx, S, 0.500, Q(0.70), Q(0.30), Q(0.20))();
        ink(ctx, S, body, WOOD_MID, 0.058);
        ctx.save(); ctx.beginPath(); body(); ctx.clip();
        on(ctx, rrect(ctx, S, Q(0.20), 0.45, Q(0.20), 0.50, 0), WOOD_LIT);   // 좌 빛 면 (4칸)
        on(ctx, rrect(ctx, S, Q(0.60), 0.45, Q(0.20), 0.50, 0), WOOD_SH);    // 우 그늘 면 (4칸)
        ctx.restore();
        /* 아가리 테 — 몸통보다 **좌우로 한 칸씩 넓게** 빼야 48px 에서 '얹힌 테'로 읽힌다(같은 폭이면
         * 몸통 윗동으로 흡수된다). 윗면 밝음 2칸 + 앞면 그늘 1칸의 두 띠. */
        ink(ctx, S, rrect(ctx, S, Q(0.15), Q(0.40), Q(0.70), Q(0.10), 0.025), WOOD_LIT, 0.052);
        on(ctx, rrect(ctx, S, Q(0.15), Q(0.45), Q(0.70), Q(0.05), 0), WOOD_SH);
        // 테 위로 넘치는 보석 3개 — 좌·우를 벌리고 가운데를 위로 올린 삼각(②의 한 칸 규칙 그대로)
        gemDot(ctx, S, 0.280, 0.290, 0.135);
        gemDot(ctx, S, 0.720, 0.290, 0.135);
        gemDot(ctx, S, 0.500, 0.155, 0.145);
        // 앞줄로 쏟아진 보석 2개
        gemDot(ctx, S, 0.135, 0.855, 0.125);
        gemDot(ctx, S, 0.865, 0.855, 0.125);
    };
    ...
    G.draw.shop_gems4 = function (ctx, S) {
        ground(ctx, S, 0.50, 0.945, 0.42, 0.045);
        // 궤짝 몸통 = 세로 널판 3장(빛·본색·그늘). 가로 이음 알파선은 걷어냈다 — 칸 스냅에서
        // 사라지고 없느니만 못한 얼룩만 남겼다(단일 슬래브 지적의 절반이 이것이었다).
        const box = () => rrect(ctx, S, Q(0.15), Q(0.55), Q(0.70), Q(0.35), 0.035)();
        ink(ctx, S, box, WOOD_MID, 0.055);
        ctx.save(); ctx.beginPath(); box(); ctx.clip();
        on(ctx, rrect(ctx, S, Q(0.15), 0.52, Q(0.20), 0.42, 0), WOOD_LIT);   // 좌 널판(빛, 4칸)
        on(ctx, rrect(ctx, S, Q(0.65), 0.52, Q(0.20), 0.42, 0), WOOD_SH);    // 우 널판(그늘, 4칸)
        ctx.restore();
        // 아가리 테 — ③과 같은 두 띠 규칙, 몸통보다 좌우 한 칸씩 넓게
        ink(ctx, S, rrect(ctx, S, Q(0.10), Q(0.45), Q(0.80), Q(0.10), 0.025), WOOD_LIT, 0.052);
        on(ctx, rrect(ctx, S, Q(0.10), Q(0.50), Q(0.80), Q(0.05), 0), WOOD_SH);
        // 아가리 위 보석 산 — 아래 3 · 꼭대기 1 (③보다 한 단 높은 피라미드)
        gemDot(ctx, S, 0.220, 0.365, 0.125);
        gemDot(ctx, S, 0.500, 0.380, 0.125);
        gemDot(ctx, S, 0.780, 0.365, 0.125);
        gemDot(ctx, S, 0.500, 0.150, 0.130);
        // 앞줄로 쏟아진 보석 2개 — ③과 같은 바닥선
        gemDot(ctx, S, 0.115, 0.870, 0.120);
        gemDot(ctx, S, 0.885, 0.870, 0.120);
    };
})(IconGen);
...
(function (G) {
    ...
    /* `stem=false` 면 **고리만** 그린다. 태엽대(0.30~1.12 = 고리 지름의 1.6배)까지 그리면
       낱개 하나가 차지하는 자리가 고리의 3배가 돼, 더미로 쌓았을 때 대가 서로를 덮어
       **주황 덩어리 하나**가 된다(실측). 더미는 고리만, 앞에 굴러나온 낱개만 대까지 그린다. */
    const winder = (ctx, S, x, y, sc, rot, stem) => {
        ctx.save();
    ...
    G.draw.shop_mount = function (ctx, S) {
        ...
        /* 넘치는 태엽 더미 — **궤짝 윗면을 통째로 덮는다.** 원본에서 윗면 초록은 한 점도 안 보이고
           태엽이 앞 모서리 위로 흘러넘친다.
           🚨 **낱개를 25개 → 9개로 줄이고 두 배 가까이 키웠다 (2026-08-25 종별 재작화).**
           종전 `sc 0.132` 는 기어 지름이 세로 기준 0.137 = **20칸 격자에서 2.7칸**이라,
           고리(0.52)·축구멍(0.23)·태엽대(0.30~1.12)가 한 칸 안에 겹쳐 뭉갰다 — 비평가가
           "주황 점 노이즈"로 부른 게 이것이다. 이 항목이 확정한 규칙(면 분할을 가진 요소는
           **최소 지름 4.4칸**)을 적용하면 `sc ≥ 0.21` 이어야 하고, 그러면 같은 자리에
           25개가 안 들어간다. **개수를 줄이는 쪽이 옳다** — 마크 텍스처는 낱알을 세지 않는다. */
        /* 🚨 **`19a52d8`(칸 색을 평균 → 최빈) 이후 한 번 더 키웠다.** 최빈 샘플링은 평균과 달리
           **부분 점수가 없다** — 한 칸을 과반으로 못 채운 색은 통째로 사라진다. `sc 0.250` 은
           축구멍 지름이 2.3칸이라 평균 시절엔 어둡게라도 남았는데, 최빈에서는 주황에 먹혀
           더미가 다시 **구멍 없는 주황 덩어리**가 됐다(실측). `sc 0.320` 이면 구멍이 **3.0칸**이라
           가운데 칸을 과반으로 채운다. 👉 **최빈 체제에서 '1칸' 규칙은 '1칸을 과반으로 채울 것'
              으로 읽어야 한다 — 즉 실효 하한이 평균 시절의 약 1.3배다.** */
        [[0.300, 0.290], [0.660, 0.270], [1.020, 0.272], [1.360, 0.300],
         [0.470, 0.450], [0.840, 0.438], [1.200, 0.452]]
            .forEach((w, i) => winder(ctx, S, w[0], w[1], 0.320, ((i * 2.399) % 6.283) - 3.14, false));
        // 앞으로 흘러나온 태엽 — 2개(종전 3개, 가장 작던 하나를 뺐다). 여기만 대까지 그린다.
        [[0.150, 0.800, 0.225, 0.6], [0.345, 0.880, 0.205, -1.2]]
