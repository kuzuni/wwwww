...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        [0.90, 0.60], [0.90, 0.50], [0.95, 0.46], [1.02, 0.50]],
            '#a8cc63', '#5f8a3a');   // R8-2: 키라인을 채움과 두 단 갈라 중경도 실루엣 레이어로 선다   /* R4: 중경 열 올리브그린 복귀(원본 정합 — 2인 치명 '회색=다른 게임'. 병렬 세션 R3 의 '담장 판독' 처방(지붕 2톤+창)은 아래에 올리브 팔레트로 이식해 병합) */
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        const stone = (x, y, w, h, sq, tilt) => {
            ...
            ctx.beginPath(); p(); ctx.fillStyle = '#161e35'; ctx.fill();   // R8-2 2인 공통 '배경보다 밝아 주목 순서가 뒤집혔다' — 하늘 아래로 침강   // R8 값게이트: 카드 하단 밴드가 원본보다 +24 떴다 — 반 단 침강   // R7: 채움을 반 단 올려 근흑 키라인이 갈리게(무선 오독 해소 — '밝다' 지적과의 균형점)   // R5 A·B 재공통 '묘비 스티커처럼 팝' — 한 단 더 침강(각인 자체는 원본 실측 사양: 해골+RIP 유지)
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        const MIDC = ['#493229', '#402c25', '#503730', '#452f28'];      // R8-2: 중경은 원본 실측 #403030(L52) 자리로 되돌리고 근흑은 근경 전용으로 — 두 층이 같은 값이면 사다리가 없다      // R8 값게이트: 원본 전경 실측 #201010(L21) 직전 단         // 색·키·크기를 함께 흔들어 타일링 패턴을 깬다
        ...
        for (let i = 0; i < 13; i++)                                       // 아래 밴드(기존 명도 + 톤 윤곽)
            figure(W * (i / 13 - 0.004 + R() * 0.050), H * (0.505 + R() * 0.050), 1.10 + R() * 0.60, MIDC[(R() * 4) | 0], pickKind(R()), 'rgba(120,86,66,.60)');   // 근흑 채움 위 어두운 윤곽은 안 보인다 — 윤곽을 채움보다 **밝게** 뒤집어 분절을 살린다
        ...
        fill(ctx, '#221208', () => ctx.rect(0, H * 0.845, W, H * 0.16));   // R8-2: 전경 인물을 원본 높이(머리 0.70H)로 올린 만큼 어깨 아래가 비어 하단 밴드가 원본보다 16~22 밝았다   // 앞 열 바탕 띠 — 이게 없으면 머리들이 '떠 있는 검은 공'이 된다(어깨가 크롭 아래라 몸이 안 이어진다)
        ...
        for (let i = 0; i < 9; i++) {
            ...
            figure(W * (i / 9 + 0.012 + R() * 0.030), H * (0.700 + R() * 0.045), 2.35 + R() * 0.45, i % 4 ? '#221208' : '#2d1a0e', [6, 6, 4, 6, 5, 1][(R() * 6) | 0], '#050302', 1.9 + R() * 0.8, H * 0.026);   // R6 B '전경 틈 제거'(밀도·어깨폭 업) · 머리가 하단 크롭에 잘리면 삼각 톱니로 읽힌다 — 0.875H · R2 공통: 전경 키라인 굵기 통일
        }
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        const tree = (x, sc) => {
            ...
            tap(x, B, x - u * 0.05, B - u * 0.30, x + u * 0.025, B - u * 0.56, u * 0.195);            // 줄기(S자 뒤틀림·위로 갈수록 가늘게·꼭대기 뾰족)
            tap(x - u * 0.015, B - u * 0.30, x - u * 0.16, B - u * 0.34, x - u * 0.30, B - u * 0.50, u * 0.100);   // 왼 큰 가지
            tap(x + u * 0.015, B - u * 0.40, x + u * 0.13, B - u * 0.50, x + u * 0.27, B - u * 0.56, u * 0.092);   // 오른 큰 가지
            tap(x + u * 0.02, B - u * 0.18, x + u * 0.10, B - u * 0.22, x + u * 0.17, B - u * 0.30, u * 0.058);    // 아래 작은 가지
