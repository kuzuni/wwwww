...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        (() => {
            /* 이번 런 비평가 B '망치에 눌려 찌그러진 나무'(탱전시): 0.645W 는 망치 머리(0.55~0.67W)
               바로 밑이라 줄기가 머리에서 삐져나온 것처럼 읽혔다 — 0.44W 로 이동(주먹 왼쪽,
               스카이라인 골 오클루전은 유지). 좌반부 휘도는 105 로 여유가 커서 안전. */
            const tx = W * 0.44;
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        bat(W * 0.775, H * 0.085, H * 0.055, -0.22);                      // (R1 공통 — 마릿수 줄이고 크기·기울기 변주: 4→3 · 활공 실루엣과 간격을 벌려 우상단 밀집 방지)
        // 언덕 2겹
        ...
        const stone = (x, y, w, h, sq) => {                                // sq=true 면 각진 어깨(형태 리듬 — 비평가 5차)
            ...
            [-0.15, 0, 0.15].forEach(dx =>                                                        // 이빨 골 3개(R3 A3 '해골인지 콘센트인지' — 하악 단서로 확정)
                fill(ctx, 'rgba(8,10,24,.60)', () => ctx.rect(x + w * (0.5 + dx - 0.030), y + w * 0.815, w * 0.060, w * 0.13)));
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        fill(ctx, '#2b1a12', () => ctx.rect(0, H * 0.925, W, H * 0.08));   // 앞 열 바탕 띠 — 이게 없으면 머리들이 '떠 있는 검은 공'이 된다(어깨가 크롭 아래라 몸이 안 이어진다)
        ...
        for (let i = 0; i < 13; i++) {
            figure(W * (i / 13 + 0.015 + R() * 0.025), H * (0.875 + R() * 0.08), 1.75 + R() * 0.8, i % 4 ? '#2b1a12' : '#38251a', [1, 4, 5][(R() * 3) | 0], '#050302', 1.6 + R() * 0.7, H * 0.026);   // 머리가 하단 크롭에 잘리면 삼각 톱니로 읽힌다 — 0.875H · 전경 키라인 굵기 통일 · R3 공통 '뿔=고양이 귀/배트맨': 앞 열에서 뿔 종(kind 2) 제외
        }
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        ink(ctx, S, bar(ctx, S, 1.872, 0.690, 1.990, 0.895, 0.100), '#2f632f', 0.030);            // 오른 위팔(아래로, 몸통보다 어둡게)
        ink(ctx, S, bar(ctx, S, 1.990, 0.895, 2.052, 1.045, 0.092), '#3d7a3b', 0.030);            // 오른 아래팔(그늘 쪽이라 왼쪽보다 어둡게)
        ...
        on(ctx, circle(ctx, S, 1.990, 0.897, 0.043), '#3d7a3b');                                  // 오른 팔꿈치
        ...
        ink(ctx, S, ell(ctx, S, 1.826, 0.462, 0.034, 0.046), '#4f9a4b', 0.026);                   // 귀 — 3/4 측면이라 얼굴 반대(오른) 실루엣 밖으로
        ...
        on(ctx, ell(ctx, S, 1.588, 0.442, 0.032, 0.038), '#e8ecd8');                              // 눈 흰자
        on(ctx, rrect(ctx, S, 1.550, 0.398, 0.076, 0.030, 0.014), 'rgba(0,0,0,.55)');             // 처진 눈꺼풀
        on(ctx, circle(ctx, S, 1.578, 0.450, 0.012), '#1c2a16');                                  // 동공(왼쪽을 본다)
        ...
        /* R3 A3·B3 공통 '팔 끝에 뼈가 붙은 듯 — 쥔 게 아니다': 뼈 자루가 주먹을 **관통**하게
           그리고(자루를 주먹 아래로 연장 + 주먹 뒤 밑동 혹 노출) 주먹을 키운다 — '감아쥔 손'이
           겹침 순서로 성립한다. */
        ink(ctx, S, ell(ctx, S, 1.512, 0.290, 0.082, 0.072, -0.48), '#4a8f47', 0.028);            // 주먹(뼈 하단을 감아쥔 타원 한 덩어리, 확대)
        /* (밑동 혹은 안 쓴다 — 머리 타래 바로 위라 '부유 구슬'로 읽혔다, 이번 런 캡처 확인.
           '쥔 손'은 망치 행 검증 해법대로 엄지+골이 말한다.) */
        ink(ctx, S, ell(ctx, S, 1.556, 0.330, 0.032, 0.026, -0.48), '#5ba455', 0.026);            // 엄지(감아쥔 쪽, 한 단 밝게)
        ctx.save();
        ctx.beginPath(); ell(ctx, S, 1.512, 0.290, 0.082, 0.072, -0.48)(); ctx.clip();
        [[1.532, 0.252], [1.548, 0.296]].forEach(([gx, gy]) =>
            on(ctx, rrect(ctx, S, gx - 0.012, gy - 0.056, 0.024, 0.112, 0.012), 'rgba(0,0,0,.26)'));   // 손가락 골 2줄(뼈 자루와 직교 방향)
        ctx.restore();
