...
const IconGen = {
    ...
    draw: {
        ...
        egg(ctx, S, o) {
            ...
            // 실루엣 키라인. **얇게** 긋는다 — 이 선이 두꺼우면 안 되는 이유가 실측으로 나왔다:
            // 🚨 이 키라인은 `OUTLINE.egg = 0.062` 가 생기기 **전에** 넣은 것이고(그때는 이게 유일한
            //    테였다), 지금은 사후 순검정 띠 0.062·S 가 실루엣 **바깥에** 따로 둘린다. 둘이 겹쳐
            //    검정 띠가 0.078·S 까지 두꺼워지고, 그 안쪽 절반이 몸통을 파먹어 **그늘 반쪽이 통째로
            //    테로 흡수**됐다(`tools/probe-egg-shade.js` + 크러시 화소 표시로 위치 확인 — 크러시는
            //    그라디언트가 아니라 **이 선의 안쪽 절반이 그리는 링**이었다. 4스톱 램프를 아무리
            //    만져도 지표가 안 움직인 게 그 증거다).
            //    두께만 0.032 → 0.012 로 줄인다. 바깥 테는 `_outlined` 가 이미 책임지므로 밝은 배경
            //    실루엣은 그대로 살아 있고(원래 이 줄이 맡던 일), 몸통은 그늘 쪽 림을 되찾는다.
            // 🚨 색도 `_shadeFloor` 로 — 순흑 키라인은 바깥 검정 띠와 구별이 안 돼 선이 아니라 띠의
            //    일부로 읽힌다(선과 면이 같은 값이면 둘 다 정보가 아니다).
            path();
