...
(function (G) {
     ...
     *    '분홍 점'으로 되돌아간다. */
    const GEM_LIT = '#ff8fae';   // 좌상 면 — RED 보다 명도 +0.29
    ...
    const gemDot = (ctx, S, x, y, r) => {
        ...
        // 좌상 스펙큘러 — 한 칸짜리 사각(삼각은 칸 스냅에 깎인다)
        on(ctx, rrect(ctx, S, x - r * 0.46, y - r * 0.44, r * 0.20, r * 0.20, 0), "#ffffff");
    };
    ...
    G.draw.shop_gems2 = function (ctx, S) {
        ...
        gemDot(ctx, S, 0.370, 0.330, 0.125);
        gemDot(ctx, S, 0.650, 0.290, 0.125);
        // 앞으로 굴러 나온 보석 1개
