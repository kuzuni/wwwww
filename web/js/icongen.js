...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        town([[-0.02, 0.74], [0.04, 0.74], [0.04, 0.60], [0.08, 0.49], [0.12, 0.60], [0.12, 0.74],
        [0.20, 0.74], [0.20, 0.62], [0.245, 0.51], [0.29, 0.62], [0.29, 0.74],
        [0.38, 0.74], [0.38, 0.64], [0.43, 0.53], [0.48, 0.64], [0.48, 0.74],
        [0.57, 0.74], [0.57, 0.60], [0.615, 0.49], [0.66, 0.60], [0.66, 0.74],
        [0.74, 0.74], [0.74, 0.64], [0.79, 0.53], [0.84, 0.64], [0.84, 0.74],
        [0.92, 0.74], [0.92, 0.62], [0.97, 0.53], [1.02, 0.60]],
            '#67a144', '#3f6c30');
        /* 근경 열 지붕/벽 분리(3차 재채점 교집합 ⓒ — 초록 단색 폴리라인은 '산울타리'로 읽힌다):
           ...
           명도만 가른다. */
        [[0.04, 0.08, 0.12, 0.60, 0.49], [0.20, 0.245, 0.29, 0.62, 0.51], [0.38, 0.43, 0.48, 0.64, 0.53],
        [0.57, 0.615, 0.66, 0.60, 0.49], [0.74, 0.79, 0.84, 0.64, 0.53], [0.92, 0.97, 1.02, 0.62, 0.53]].forEach(([lx, px, rx2, by, py]) => {
            fill(ctx, '#3f6c30', () => { ctx.moveTo(W * lx, H * by); ctx.lineTo(W * px, H * py); ctx.lineTo(W * rx2, H * by); });
            fill(ctx, '#548538', () => { ctx.moveTo(W * lx, H * by); ctx.lineTo(W * px, H * py); ctx.lineTo(W * px, H * by); });   // 빛 받는 왼 지붕면(2톤)
        });
        /* 창 실루엣(10차 2인 공통 '마을 판독') — 근경 열 벽면에 어두운 창 5개 */
        [[0.065, 0.655], [0.235, 0.665], [0.425, 0.67], [0.615, 0.655], [0.795, 0.665]].forEach(([wx, wy]) =>
            fill(ctx, '#3f6c30', () => ctx.rect(W * wx, H * wy, W * 0.020, H * 0.038)));
        /* 굴뚝(9차 '마을이 아니라 도시 실루엣') — 근경 열 지붕 경사 위 작은 굴뚝 3개 */
        [[0.075, 0.565], [0.44, 0.605], [0.805, 0.605]].forEach(([cx, cy]) => {
            fill(ctx, '#67a144', () => ctx.rect(W * cx, H * (cy - 0.055), W * 0.017, H * 0.055));
            ctx.strokeStyle = '#3f6c30'; ctx.lineWidth = H * 0.014;
            ctx.strokeRect(W * cx, H * (cy - 0.055), W * 0.017, H * 0.055);
            fill(ctx, '#3f6c30', () => ctx.rect(W * (cx - 0.004), H * (cy - 0.068), W * 0.025, H * 0.016));
        });
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        sky(ctx, W, H, '#171e3e', '#080b1c');   /* R5 B(2연속) '원본은 거의 검정 암청' — 한 단 감광(title-ink 좌반 하한 26 주의: 이 이상 내리지 말 것) */
        ...
        const stone = (x, y, w, h, sq) => {                                // sq=true 면 각진 어깨(형태 리듬 — 비평가 5차)
            ...
            ctx.beginPath(); p(); ctx.fillStyle = '#293754'; ctx.fill();   // R5 A·B 재공통 '묘비 스티커처럼 팝' — 한 단 더 침강(각인 자체는 원본 실측 사양: 해골+RIP 유지)
        ...
        (() => {
            ...
            for (let i = 0; i < 9; i++)                                    // 잎 덩어리(자리 먼저 뽑는다 — 스트로크→채움 2패스용)
                ...
                blobs.push([x + H * (-0.22 + R() * 0.50), H * (-0.08 + R() * 0.30), H * (0.12 + R() * 0.08), i % 2 ? '#20452c' : '#265238']);
        ...
        (() => {
            ...
            for (let i = 0; i < 5; i++)                                    // 작은 캐노피(좌측 나무와 같은 어두운 초록 계열)
                blobs.push([x + H * (-0.16 + R() * 0.24), H * (0.525 + R() * 0.07), H * (0.05 + R() * 0.032), i % 2 ? '#16301f' : '#1c3a28']);
            ctx.strokeStyle = '#060b12'; ctx.lineWidth = H * 0.024;
            blobs.forEach(([lx, ly, r]) => { ctx.beginPath(); ctx.arc(lx, ly, r, 0, Math.PI * 2); ctx.stroke(); });
            blobs.forEach(([lx, ly, r, c]) => fill(ctx, c, () => ctx.arc(lx, ly, r, 0, Math.PI * 2)));
            fill(ctx, '#2a4f33', () => ctx.arc(x - H * 0.03, H * 0.525, H * 0.028, 0, Math.PI * 2));   // 달빛 림 혹 하나
        })();
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        (() => {
            ...
            while (x < W) {
                const r = H * (0.022 + R() * 0.034), cy = H * (0.393 + R() * 0.022);
        ...
        for (let i = 0; i < 16; i++)                                       // 위 밴드(밝게)
            figure(W * (i / 16 - 0.004 + R() * 0.036), H * (0.530 + R() * 0.045), 0.52 + R() * 0.62, MIDC_HI[(R() * 4) | 0], pickKind(R()));
        for (let i = 0; i < 22; i++)                                       // 아래 밴드(기존 명도 + 톤 윤곽)
            figure(W * (i / 22 - 0.004 + R() * 0.033), H * (0.585 + R() * 0.060), 0.66 + R() * 0.78, MIDC[(R() * 4) | 0], pickKind(R()), 'rgba(74,45,32,.55)');
        ...
        banner(W * 0.30, H * 0.08, H * 0.28, H * 0.30, -0.20, '#4a3527');  // R5 B '원본은 검정 깃발': 밝은 갈색 2장 근흑화(암적 악센트 2장은 3차 사양 유지)
        banner(W * 0.585, H * 0.02, H * 0.26, H * 0.28, -0.14, '#3e2b20');
        ...
        fill(ctx, '#221208', () => ctx.rect(0, H * 0.925, W, H * 0.08));   // 앞 열 바탕 띠 — 이게 없으면 머리들이 '떠 있는 검은 공'이 된다(어깨가 크롭 아래라 몸이 안 이어진다)
        ...
        for (let i = 0; i < 13; i++) {
            ...
            figure(W * (i / 13 + 0.015 + R() * 0.025), H * (0.875 + R() * 0.08), 1.75 + R() * 0.8, i % 4 ? '#221208' : '#2d1a0e', [6, 6, 4, 6, 5, 1][(R() * 6) | 0], '#050302', 1.6 + R() * 0.7, H * 0.026);   // 머리가 하단 크롭에 잘리면 삼각 톱니로 읽힌다 — 0.875H · R2 공통: 전경 키라인 굵기 통일
        }
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        const band = (bx, by, bw, bh2, c) => {
            ...
            /* R5 A [치명] '이 화면 유일의 유선 하늘 요소 — 라인 정책 충돌': 검은 키라인을 걷고
               하늘보다 어두운 보라 실루엣으로(원본 실측도 무외곽선 저명도 밴드 — R4 A 의 '유선
               구름' 처방이 원본 이탈이었다). 뭉게 혹 실루엣은 유지. */
            fill(ctx, c, p);
        };
        band(W * 0.115, H * 0.15, W * 0.115, H * 0.052, '#a683c8');
        band(W * 0.45, H * 0.10, W * 0.085, H * 0.046, '#9d7ac0');
        band(W * 0.345, H * 0.26, W * 0.065, H * 0.040, '#a683c8');
        ...
        const tree = (x, sc) => {
            ...
            /* R5 A '중간 명도 보라 나무 — 원본은 거의 검은 자주 실루엣': 파스텔 하늘 전환(R4)으로
               상대 명도가 떠서 한 단 침강 */
            const c = '#221530', oc = '#0d071c', u = H * sc, B = H * 0.86;
            const P = [];
        ...
        const drawRemains = () => {
            ...
            /* 이번 런 B '해골 옆 회색 덩어리 판독 불능': 갈비 아치가 굵은 키라인에 뭉개져 요철만
               남았다 — 키라인을 가늘게, 아치 반경을 키우고 간격을 벌려 **늑골 사이 틈**이 1x 에서
               보이게 한다. 척추 끝에 골반 혹을 달아 '유해'의 형태 언어를 명시. */
            [[H * 0.040, '#150e26'], [H * 0.024, '#d9d2e6']].forEach(([lw, col]) => {
                ctx.strokeStyle = col; ctx.lineWidth = lw; ctx.lineCap = 'round';
                ctx.beginPath(); ctx.moveTo(sx + r * 1.0, sy + r * 0.55); ctx.lineTo(sx + r * 3.55, sy + r * 0.62); ctx.stroke();   // 척추
                [1.55, 2.35, 3.10].forEach(k => {                                                 // 갈비 아치(간격을 벌려 틈이 보이게)
                    ctx.beginPath(); ctx.arc(sx + r * k, sy + r * 0.58, r * 0.60, Math.PI, Math.PI * 2); ctx.stroke();
                });
                ctx.lineCap = 'butt';
            });
            (() => {                                                                              // 골반 혹(척추 끝)
                const px = sx + r * 3.55, py = sy + r * 0.62;
                ctx.beginPath(); ctx.ellipse(px, py, r * 0.30, r * 0.24, 0.2, 0, Math.PI * 2);
                ctx.strokeStyle = '#150e26'; ctx.lineWidth = H * 0.020; ctx.stroke();
                ctx.beginPath(); ctx.ellipse(px, py, r * 0.30, r * 0.24, 0.2, 0, Math.PI * 2);
                ctx.fillStyle = '#d9d2e6'; ctx.fill();
            })();
