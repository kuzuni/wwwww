...
(function (G) {
    ...
    G.draw.dg_hammer = function (ctx, S) {
        ...
        const town = (pts, fc, oc) => {
            ...
            ctx.strokeStyle = oc; ctx.lineWidth = H * 0.020; ctx.stroke();
        };
        ...
        town([[-0.02, 0.56], [0.05, 0.56], [0.05, 0.44], [0.09, 0.38], [0.13, 0.44], [0.13, 0.56],
        [0.22, 0.56], [0.22, 0.42], [0.265, 0.36], [0.31, 0.42], [0.31, 0.56],
        [0.40, 0.56], [0.40, 0.46], [0.45, 0.40], [0.50, 0.46], [0.50, 0.56],
        [0.60, 0.56], [0.60, 0.40], [0.645, 0.34], [0.69, 0.40], [0.69, 0.56],
        [0.79, 0.56], [0.79, 0.44], [0.845, 0.38], [0.90, 0.44], [0.90, 0.56], [1.02, 0.56]],
            '#c9d6d8', '#a7bac2');
        town([[-0.02, 0.60], [0.03, 0.60], [0.03, 0.46], [0.055, 0.40], [0.08, 0.46], [0.08, 0.60],
        [0.13, 0.60], [0.13, 0.50], [0.155, 0.50], [0.155, 0.34], [0.185, 0.28], [0.215, 0.34], [0.215, 0.50], [0.24, 0.50], [0.24, 0.60],
        [0.30, 0.60], [0.30, 0.44], [0.335, 0.38], [0.37, 0.44], [0.37, 0.60],
        [0.43, 0.60], [0.43, 0.50], [0.475, 0.50], [0.475, 0.30], [0.505, 0.24], [0.535, 0.30], [0.535, 0.50], [0.565, 0.50], [0.565, 0.60],
        [0.63, 0.60], [0.63, 0.42], [0.665, 0.36], [0.70, 0.42], [0.70, 0.60],
        ...
        [0.57, 0.74], [0.57, 0.60], [0.615, 0.54], [0.66, 0.60], [0.66, 0.74],
        [0.74, 0.74], [0.74, 0.64], [0.79, 0.58], [0.84, 0.64], [0.84, 0.74],
        [0.92, 0.74], [0.92, 0.62], [0.97, 0.58], [1.02, 0.60]],
            '#71a94c', '#4c7a3a');
        /* 근경 열 지붕/벽 분리(3차 재채점 교집합 ⓒ — 초록 단색 폴리라인은 '산울타리'로 읽힌다):
           ...
           명도만 가른다. */
        [[0.04, 0.08, 0.12, 0.60, 0.54], [0.20, 0.245, 0.29, 0.62, 0.56], [0.38, 0.43, 0.48, 0.64, 0.58],
        [0.57, 0.615, 0.66, 0.60, 0.54], [0.74, 0.79, 0.84, 0.64, 0.58], [0.92, 0.97, 1.02, 0.62, 0.58]].forEach(([lx, px, rx2, by, py]) => {
            fill(ctx, '#4c7a3a', () => { ctx.moveTo(W * lx, H * by); ctx.lineTo(W * px, H * py); ctx.lineTo(W * rx2, H * by); });
            fill(ctx, '#5f9142', () => { ctx.moveTo(W * lx, H * by); ctx.lineTo(W * px, H * py); ctx.lineTo(W * px, H * by); });   // 빛 받는 왼 지붕면(2톤)
        });
        /* 창 실루엣(10차 2인 공통 '마을 판독') — 근경 열 벽면에 어두운 창 5개 */
        [[0.065, 0.655], [0.235, 0.665], [0.425, 0.67], [0.615, 0.655], [0.795, 0.665]].forEach(([wx, wy]) =>
            fill(ctx, '#4c7a3a', () => ctx.rect(W * wx, H * wy, W * 0.020, H * 0.038)));
        /* 굴뚝(9차 '마을이 아니라 도시 실루엣') — 근경 열 지붕 경사 위 작은 굴뚝 3개 */
        [[0.075, 0.565], [0.44, 0.605], [0.805, 0.605]].forEach(([cx, cy]) => {
            fill(ctx, '#71a94c', () => ctx.rect(W * cx, H * (cy - 0.055), W * 0.017, H * 0.055));
            ctx.strokeStyle = '#4c7a3a'; ctx.lineWidth = H * 0.014;
            ctx.strokeRect(W * cx, H * (cy - 0.055), W * 0.017, H * 0.055);
            fill(ctx, '#4c7a3a', () => ctx.rect(W * (cx - 0.004), H * (cy - 0.068), W * 0.025, H * 0.016));
        });
        /* 중경 나무 한 그루(잔여 결함 ⓓ — 오클루전 한 겹): 마을 지붕선 **앞**에 서서 건물 일부를
           가린다 — 겹침이 없으면 스카이라인·지면이 무대 배경막처럼 분리돼 보인다(8차 지적).
           좌반부 휘도(title-ink)를 안 깎게 x 0.64W(오른쪽 절반), 캐노피는 지붕선(0.54~0.62H)에 걸치는
           높이만. 뒤이어 그리는 ground(0.82H)가 뿌리를 덮어 접지는 자연히 해결된다. */
        (() => {
            const tx = W * 0.645;
            [[H * 0.040 + H * 0.026, '#12300f'], [H * 0.040, '#5d4030']].forEach(([lw, c]) => {   // 줄기(키라인 언더레이 — R2 공통 '전경 키라인 비일관')
                ctx.strokeStyle = c; ctx.lineCap = 'round'; ctx.lineWidth = lw;
                ctx.beginPath(); ctx.moveTo(tx + H * 0.02, H * 0.86); ctx.quadraticCurveTo(tx - H * 0.02, H * 0.70, tx, H * 0.56); ctx.stroke();
                ctx.lineCap = 'butt';
            });
            const blobs = [];
            for (let i = 0; i < 4; i++)                                    // 캐노피(아래 어둡게)
                blobs.push([tx + H * (-0.11 + R() * 0.22), H * (0.485 + R() * 0.075), H * (0.065 + R() * 0.04), i % 2 ? '#2f7a2f' : '#256c28']);
            for (let i = 0; i < 3; i++)                                    // 윗부분 밝은 혹 + 림
                blobs.push([tx + H * (-0.08 + R() * 0.16), H * (0.465 + R() * 0.045), H * (0.035 + R() * 0.025), i === 2 ? '#79c257' : '#4b9a3c']);
            ctx.strokeStyle = '#12300f'; ctx.lineWidth = H * 0.026;        // 합집합 키라인(덤불 화법 — 채움이 안쪽 획을 덮는다)
        ...
        ground(ctx, W, H, H * 0.82, '#59a53c');
        ground(ctx, W, H, H * 0.91, '#3f8a2c');
        for (let i = 0; i < 26; i++) {                                    // 풀포기
            const x = R() * W, y = H * (0.84 + R() * 0.14), h = H * (0.05 + R() * 0.05);
        ...
        (() => {
            ...
            [[H * 0.026, '#12300f'], [0, '#5d4030']].forEach(([pad, c]) => {   // 줄기 2획(키라인 언더레이 — R2 공통)
                ...
                ctx.beginPath(); ctx.moveTo(tx + H * 0.10, H * 1.02); ctx.quadraticCurveTo(tx - H * 0.06, H * 0.62, tx - H * 0.02, H * 0.30); ctx.stroke();
            ...
            ctx.strokeStyle = '#12300f'; ctx.lineWidth = H * 0.028;       // 캐노피 합집합 키라인(R2 공통 — 채움이 안쪽 획을 덮는다)
            blobs.forEach(([x, y, r]) => { ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.stroke(); });
            blobs.forEach(([x, y, r, c]) => fill(ctx, c, () => ctx.arc(x, y, r, 0, Math.PI * 2)));
            for (let i = 0; i < 7; i++) {                                 // 연두 림라이트 혹(키라인 없이 맨 위)
                const x = W * 0.86 + H * (-0.30 + R() * 0.52), y = H * (-0.10 + R() * 0.26), r = H * (0.045 + R() * 0.035);
        ...
        bush(W * 0.04, H * 0.98, 1.5, '#3f8a2c', '#102a10');
        bush(W * 0.115, H * 1.05, 2.0, '#35802a', '#102a10');
        bush(W * 0.30, H * 1.06, 1.0, '#3f8a2c', '#102a10');
        ...
        [-0.052, 0.026].forEach(y =>                                                      // 손가락 골(자루와 직교, 2줄만)
            on(ctx, rrect(ctx, S, -0.104, y, 0.204, 0.030, 0.015), 'rgba(0,0,0,.28)'));
        ...
        on(ctx, rrect(ctx, S, -0.102, 0.132, 0.204, 0.038, 0.019), 'rgba(0,0,0,.12)');    // 주먹 밑 접힘 음영 한 줄(경계는 명도로만)
        ink(ctx, S, rrect(ctx, S, -0.135, 0.238, 0.270, 0.580, 0.060), '#7d5a8f', 0.030); // 소매(보라 튜닉) — 프레임 하단 크롭 밖까지
        on(ctx, rrect(ctx, S, -0.135, 0.238, 0.270, 0.075, 0.036), 'rgba(255,255,255,.20)'); // 소매 접힘 밴드(팔꿈치 쪽 광)
        on(ctx, rrect(ctx, S, -0.135, 0.560, 0.270, 0.258, 0.060), 'rgba(0,0,0,.16)');    // 소매 하단 그늘(플랫 사다리꼴 지적 — 명암 한 단)
        ctx.restore();
        ctx.save();                                                                       // 머리 — 자루 끝(우상단)
        // 머리 상자는 세로(local y)가 장축 = 자루와 직교하려면 회전각이 자루 기울기 그대로여야 한다.
        ctx.translate(2.100 * S, 0.310 * S); ctx.rotate(SWING);
        /* R1 비평가 2인 공통 '망치 머리 은색 = 원본 아이덴티티 손실': 원본 실측은 어두운 올리브+
           금색 계열이다 — 은회(#7b838a, 9차 '녹색 위 녹색' 회피용)를 **어두운 올리브 2톤**으로
           복귀시키되, 9차 함정은 명도로 피한다: 마을 초록(#71a94c·#4c7a3a)보다 두 단 어두운
           올리브라 실루엣이 붙지 않고, 머리가 걸리는 배경도 대부분 하늘이다. 금은 쐐기 못이 맡는다. */
        ink(ctx, S, rrect(ctx, S, -0.160, -0.255, 0.320, 0.510, 0.050), '#5f6434', 0.032);
        ctx.save();
        ctx.beginPath(); rrect(ctx, S, -0.160, -0.255, 0.320, 0.510, 0.050)(); ctx.clip();
        /* ⚠️ 하이라이트 띠+물림 테 2줄+결 줄+스펙큘러를 다 얹으면 가로세로 선이 교차해 **타탄
           격자 직물**로 읽힌다(비평가 6차 2인 공통 1순위 — 확대 크롭으로 확인). 3면 셰이딩과
           물림 테 하나만 남긴다. */
        /* R2 비평가 2인 공통 '단일 면 + 대각 명도 밴드 = 그라디언트 스티커': 원본은 윗면·정면이
           갈리는 2면 베벨 — 밝은 윗면(+x 절반)과 정면(바탕색)을 **하드 에지 한 줄**로 딱 가른다.
           3단 밴드(빛면·중간·그늘) 금지 — 두 면이면 입체, 세 밴드면 그라디언트다. */
        on(ctx, rrect(ctx, S, 0.008, -0.255, 0.152, 0.510, 0), '#7e8447');                // 윗면(빛면) — local +x 가 회전 후 우상향
        on(ctx, rrect(ctx, S, -0.002, -0.255, 0.020, 0.510, 0), 'rgba(0,0,0,.45)');       // 면 경계 하드 에지
        on(ctx, rrect(ctx, S, -0.160, 0.203, 0.320, 0.052, 0), 'rgba(0,0,0,.22)');        // 자루 쪽 타격면의 얕은 그늘(면 경계와 직교 — 베벨 힌트)
    ...
    G.draw.dg_ghost = function (ctx, S) {
        ...
        const streak = (sx, sy, len, th, c) => {
            ctx.beginPath();
            ctx.moveTo(sx - th * 3.0, sy + th * 0.50);                     // 뾰족한 왼 꼬리
            ctx.quadraticCurveTo(sx - th * 0.8, sy + th * 0.05, sx, sy);
            let x = sx;
            while (x < sx + len) {                                         // 윗변 — 반경 편차 큰 혹
                const seg = H * (0.08 + R() * 0.14), r = th * (0.35 + R() * 1.25);
                ctx.quadraticCurveTo(x + seg * 0.5, sy - r * 2, x + seg, sy - r * 0.15);
                x += seg;
            }
            ctx.quadraticCurveTo(x + th * 1.2, sy + th * 0.10, x + th * 3.2, sy + th * 0.40);   // 뾰족한 오른 꼬리
            while (x > sx) {                                               // 아랫변 — 뾰족 찢김
                const seg = H * (0.07 + R() * 0.10);
                ctx.lineTo(x - seg * 0.35, sy + th * (0.55 + R() * 0.85));
                ctx.lineTo(x - seg, sy + th * (0.30 + R() * 0.45));
                x -= seg;
            }
        ...
        const mx = W * 0.58, my = H * 0.34, mr = H * 0.21;                // 달
        fill(ctx, 'rgba(190,200,225,.13)', () => ctx.arc(mx, my, mr * 1.42, 0, Math.PI * 2));   // 회청 헤일로 원반
        ...
        ctx.stroke();
        ctx.lineCap = 'butt';
        fill(ctx, 'rgba(120,100,55,.16)', () => ctx.arc(mx + mr * 0.52, my - mr * 0.30, mr * 0.16, 0, Math.PI * 2));   // 크레이터(보조)
        fill(ctx, 'rgba(120,100,55,.14)', () => ctx.arc(mx - mr * 0.48, my + mr * 0.42, mr * 0.12, 0, Math.PI * 2));
        ctx.restore();
        ...
        bat(W * 0.575, H * 0.20, H * 0.105, 0);                           // 달 원반 위쪽(R2 공통 '구름 띠와 융합 = 뿔 달린 덩어리' — 띠에서 떼어 냈다)
        bat(W * 0.522, H * 0.46, H * 0.080, 0.28);                        // 달 왼 가장자리에 걸쳐 원반을 파먹는다(오클루전)
        bat(W * 0.68, H * 0.13, H * 0.055, -0.22);                        // (R1 공통 — 마릿수 줄이고 크기·기울기 변주: 4→3)
        // 언덕 2겹
        ...
        (() => {
            ...
            const hx = W * 0.30, hy = H * 0.625, hw = W * 0.105, hh = H * 0.175;
            const house = () => {
        ...
        const stone = (x, y, w, h, sq) => {                                // sq=true 면 각진 어깨(형태 리듬 — 비평가 5차)
            const p = sq
            ...
            ctx.beginPath(); p(); ctx.strokeStyle = '#05071a'; ctx.lineWidth = H * 0.030; ctx.stroke();   // R2 공통 '전경 키라인 비일관' — 굵은 근흑 키라인(스트로크→채움 순서로 바깥만 남긴다)
            ctx.beginPath(); p(); ctx.fillStyle = '#3d4a72'; ctx.fill();   // 밝은 회청은 밤 무드에서 튄다(비평가 6차) — 한 단 가라앉힘
            fill(ctx, 'rgba(8,10,24,.9)', () => ctx.arc(x + w * 0.36, y + w * 0.52, w * 0.13, 0, Math.PI * 2));             // 해골 각인(두 눈 + 코)
            fill(ctx, 'rgba(8,10,24,.9)', () => ctx.arc(x + w * 0.64, y + w * 0.52, w * 0.13, 0, Math.PI * 2));
            fill(ctx, 'rgba(8,10,24,.75)', () => { ctx.moveTo(x + w * 0.5, y + w * 0.62); ctx.lineTo(x + w * 0.42, y + w * 0.78); ctx.lineTo(x + w * 0.58, y + w * 0.78); });
            fill(ctx, 'rgba(8,10,24,.7)', () => ctx.rect(x + w * 0.22, y + w * 0.94, w * 0.56, h * 0.09));                  // RIP 줄 각인
            fill(ctx, 'rgba(8,10,24,.55)', () => ctx.rect(x + w * 0.30, y + w * 0.94 + h * 0.16, w * 0.40, h * 0.07));
            fill(ctx, 'rgba(255,255,255,.10)', () => ctx.rect(x + w * 0.10, y + w * 0.30, w * 0.09, h * 0.50));             // 왼쪽 세로 하이라이트
        };
        stone(W * 0.475, H * 0.70, H * 0.17, H * 0.30, true);              // 셋째(작게, 각진 어깨) — 바닥 허전함 보완(비평가 4차)
        stone(W * 0.585, H * 0.62, H * 0.22, H * 0.40);
        stone(W * 0.705, H * 0.56, H * 0.26, H * 0.48, true);              // 큰 것은 각짐/둥긂을 섞어 리듬(비평가 5차) · 3차 교집합 ⓔ: [열기]에 잘리던 자리에서 ~10px 왼쪽으로
        ...
        (() => {
            ...
            trunkStroke(t + H * 0.036, '#0a0614'); trunkStroke(t, '#2e2119');
            /* ⚠️ 잎을 너무 검게(#101f16) 깔면 남색 하늘과 명도차가 없어 '잎 없는 마른 줄기'로
               읽힌다(비평가 7차) — 한 단 밝은 초록으로 캐노피 덩어리를 세운다. 좌반부 평균 휘도
               하한(26)은 밝아지는 쪽이라 안전하다. */
            const blobs = [];
            for (let i = 0; i < 9; i++)                                    // 잎 덩어리(자리 먼저 뽑는다 — 스트로크→채움 2패스용)
                blobs.push([x + H * (-0.22 + R() * 0.50), H * (-0.08 + R() * 0.30), H * (0.12 + R() * 0.08), i % 2 ? '#1c3a26' : '#224630']);
            ctx.strokeStyle = '#060b12'; ctx.lineWidth = H * 0.028;        // 합집합 키라인(채움이 안쪽 획을 덮는다 — 덤불 화법)
        ...
        (() => {
            ...
            for (let i = 0; i < 5; i++)                                    // 작은 캐노피(좌측 나무와 같은 어두운 초록 계열)
                blobs.push([x + H * (-0.16 + R() * 0.24), H * (0.525 + R() * 0.07), H * (0.05 + R() * 0.032), i % 2 ? '#1c3a26' : '#224630']);
            ctx.strokeStyle = '#060b12'; ctx.lineWidth = H * 0.024;
            blobs.forEach(([lx, ly, r]) => { ctx.beginPath(); ctx.arc(lx, ly, r, 0, Math.PI * 2); ctx.stroke(); });
            blobs.forEach(([lx, ly, r, c]) => fill(ctx, c, () => ctx.arc(lx, ly, r, 0, Math.PI * 2)));
            fill(ctx, '#33603c', () => ctx.arc(x - H * 0.03, H * 0.525, H * 0.028, 0, Math.PI * 2));   // 달빛 림 혹 하나
        })();
    ...
    G.draw.dg_invasion = function (ctx, S) {
        ...
        sky(ctx, W, H, '#f7e6c6', '#d98f5c');   // 지평선을 진한 주황으로 — 옅으면 새벽 무드가 죽는다(비평가 5차)
        ...
        (() => {
            ctx.beginPath(); ctx.moveTo(0, H); ctx.lineTo(0, H * 0.40);
            let x = 0;
        ...
        const figure = (x, y, s, c, kind, oc, wf, ow) => {
            ...
            const paths = [];
            if (kind === 1) paths.push(() => { ctx.moveTo(x - r, y + r * 0.5); ctx.lineTo(x, y - r * 1.15); ctx.lineTo(x + r, y + r * 0.5); ctx.closePath(); });
                ...
                ctx.quadraticCurveTo(x + r * 0.95, y - r * 1.18, x + r * 1.30, y - r * 0.62);   // 처진 꼭지
                ctx.quadraticCurveTo(x + r * 1.02, y - r * 0.30, x + r * 0.96, y + r * 0.25);
                ctx.quadraticCurveTo(x + r * 0.85, y + r * 0.6, x + r * 0.7, y + r * 0.6);
                ctx.closePath();
            });
            else if (kind === 5) {                                         // 챙 투구 — 사다리꼴 + 넓은 챙
                ...
                paths.push(() => ctx.rect(x - r * 1.12, y + r * 0.22, r * 2.24, r * 0.30));
            }
            else paths.push(() => ctx.arc(x, y, r, 0, Math.PI * 2));       // 0·2·4 는 둥근 기본머리
            if (kind === 2) {                                              // 양쪽 뿔 — 크고 잦으면 '고양이 귀'로 읽힌다(비평가 4차): 짧게
                paths.push(() => { ctx.moveTo(x - r * 0.7, y - r * 0.2); ctx.lineTo(x - r * 1.15, y - r * 1.0); ctx.lineTo(x - r * 0.15, y - r * 0.85); ctx.closePath(); });
                paths.push(() => { ctx.moveTo(x + r * 0.7, y - r * 0.2); ctx.lineTo(x + r * 1.15, y - r * 1.0); ctx.lineTo(x + r * 0.15, y - r * 0.85); ctx.closePath(); });
            }
            if (kind === 4)                                                // 볏 — 정수리 지느러미 한 장
                paths.push(() => { ctx.moveTo(x - r * 0.50, y - r * 0.72); ctx.quadraticCurveTo(x + r * 0.05, y - r * 1.75, x + r * 0.58, y - r * 0.72); ctx.closePath(); });
        ...
        for (let i = 0; i < 14; i++)                                       // 원경-중경 사이 반층 — 레이어 한 겹 더(비평가 5차)
            figure(W * (i / 14 + 0.015 + R() * 0.03), H * (0.465 + R() * 0.04), 0.55 + R() * 0.25, '#b79a82', R() < 0.6 ? 0 : 1);
        ...
        for (let i = 0; i < 16; i++)                                       // 위 밴드(밝게)
            figure(W * (i / 16 - 0.004 + R() * 0.036), H * (0.530 + R() * 0.045), 0.62 + R() * 0.45, MIDC_HI[(R() * 4) | 0], pickKind(R()));
        for (let i = 0; i < 22; i++)                                       // 아래 밴드(기존 명도 + 톤 윤곽)
            figure(W * (i / 22 - 0.004 + R() * 0.033), H * (0.585 + R() * 0.060), 0.78 + R() * 0.58, MIDC[(R() * 4) | 0], pickKind(R()), 'rgba(74,45,32,.55)');
        ...
        banner(W * 0.30, H * 0.08, H * 0.28, H * 0.30, -0.20, '#7d5c4e');  // 중경 밝은 배너(군중 앞이라 깊이가 갈린다)
        banner(W * 0.585, H * 0.02, H * 0.26, H * 0.28, -0.14, '#6d5044');
        banner(W * 0.05, H * 0.02, H * 0.30, H * 0.34, 0.06, '#5a2019');   /* 9차 '배너 복붙' — 색을 4종으로 갈랐다 · 3차 교집합 ⓔ: 1장은 암적 악센트(전장 서사 + 팔레트 포인트) */
        banner(W * 0.14, -H * 0.10, H * 0.40, H * 0.42, 0.10, '#2e1c15');
        banner(W * 0.46, -H * 0.14, H * 0.38, H * 0.40, -0.08, '#241511');
        banner(W * 0.70, -H * 0.05, H * 0.36, H * 0.40, 0.16, '#471810');  // 3차 교집합 ⓔ: 암적 두 번째(좌우로 벌려 반복감 방지)
        ...
        fill(ctx, '#241612', () => ctx.rect(0, H * 0.925, W, H * 0.08));   // 앞 열 바탕 띠 — 이게 없으면 머리들이 '떠 있는 검은 공'이 된다(어깨가 크롭 아래라 몸이 안 이어진다)
        for (let i = 0; i < 13; i++) {                                     // 앞 열 — 키라인 + 폭 변주(균일 반원 반복 = 스캘럽 무늬, 비평가 6차)
            figure(W * (i / 13 + 0.015 + R() * 0.025), H * (0.875 + R() * 0.08), 1.75 + R() * 0.8, i % 4 ? '#241612' : '#2e1d16', pickKind(R()), '#050302', 1.6 + R() * 0.7, H * 0.026);   // 머리가 하단 크롭에 잘리면 삼각 톱니로 읽힌다 — 0.875H · R2 공통: 전경 키라인 굵기 통일
        }
    ...
    G.draw.dg_zombie = function (ctx, S) {
        ...
        /* 9차 2인 공통 '스켈레톤 로딩 바' 오독: 원본이 수평 구름 밴드인 건 실측 확정이라 밴드는
           유지하되, **윗변에 뭉게 혹 2개**를 얹어 '구름'으로 판독되게 한다(균일 캡슐 금지). */
        const band = (bx, by, bw, bh2, c) => fill(ctx, c, () => {
            ...
            ctx.closePath();
            ctx.arc(bx + bw * 0.30, by, bh2 * 0.72, 0, Math.PI * 2);       // 뭉게 혹(왼쪽 크게)
            ctx.arc(bx + bw * 0.62, by + bh2 * 0.10, bh2 * 0.50, 0, Math.PI * 2);   // 뭉게 혹(오른쪽 작게)
        });
        ...
        fill(ctx, '#5f4585', () => { ctx.moveTo(0, H * 0.66); ctx.quadraticCurveTo(W * 0.22, H * 0.575, W * 0.46, H * 0.645); ctx.quadraticCurveTo(W * 0.72, H * 0.71, W, H * 0.625); ctx.lineTo(W, H); ctx.lineTo(0, H); });
        fill(ctx, '#533a74', () => { ctx.moveTo(0, H * 0.735); ctx.quadraticCurveTo(W * 0.30, H * 0.655, W * 0.58, H * 0.72); ctx.quadraticCurveTo(W * 0.82, H * 0.775, W, H * 0.705); ctx.lineTo(W, H); ctx.lineTo(0, H); });
        ...
        const tree = (x, sc) => {                                          // 고목 — 줄기 + 갈라진 가지 + 잔가지(뒤틀린 질감, 비평가 5차)
            /* R2 비평가 2인 공통 '옷걸이/사슴뿔 막대기 + 키라인 없음': 줄기·가지를 근흑 언더레이
               위에 겹쳐 그려 키라인을 세우고, 줄기에 옹이 하나·가지 한 쌍을 더해 고목의 뒤틀림을
               올린다(색은 실루엣 사양 유지). */
            ...
            const arms = (col, pad) => {
                ctx.lineCap = 'round';
                const arm = (dx, dy, len, w) => {
                    const ex = x + dx * len, ey = H * (0.86 - (0.42 + dy) * sc);
                    ctx.beginPath();
                    ctx.moveTo(x, H * (0.86 - 0.42 * sc));
                    ctx.quadraticCurveTo(x + dx * len * 0.55, H * (0.86 - (0.40 + dy * 0.6) * sc), ex, ey);
                    ctx.strokeStyle = col; ctx.lineWidth = t * w + pad; ctx.stroke();
                    [[0.6, -0.10], [-0.3, -0.14]].forEach(([tx, ty]) => {   // 끝 잔가지 두 갈래
                        ctx.beginPath(); ctx.moveTo(ex, ey);
                        ctx.quadraticCurveTo(ex + dx * H * 0.04 * sc, ey + H * ty * 0.4 * sc, ex + dx * H * 0.07 * sc * (1 + tx * 0.4), ey + H * ty * sc);
                        ctx.lineWidth = t * w * 0.45 + pad; ctx.stroke();
                    });
                };
                arm(-1, 0.18, H * 0.22 * sc, 0.85); arm(1, 0.22, H * 0.26 * sc, 0.80);
                arm(-1, 0.34, H * 0.13 * sc, 0.60); arm(1, 0.36, H * 0.15 * sc, 0.55);
                arm(-1, 0.06, H * 0.11 * sc, 0.50); arm(1, 0.08, H * 0.10 * sc, 0.45);   // 아래 가지 한 쌍(포크 2갈래 탈피)
                ctx.lineCap = 'butt';
            };
            arms(oc, H * 0.022);                                           // 가지 키라인 언더레이
            ctx.beginPath(); trunk(); ctx.strokeStyle = oc; ctx.lineWidth = H * 0.024; ctx.stroke();   // 줄기 키라인
            arms(c, 0);
            fill(ctx, c, trunk);
            fill(ctx, oc, () => ctx.ellipse(x + t * 0.15, H * (0.86 - 0.26 * sc), t * 0.22, t * 0.34, 0.2, 0, Math.PI * 2));   // 옹이
        };
        ...
        const drawRemains = () => {
            ...
            /* 뼈 무더기 키라인(R2 공통 '전경 키라인 비일관'): 척추·갈비를 근흑 언더레이 위에 겹쳐
               그리고 두개골·턱도 스트로크→채움 순서로 키라인을 두른다. */
            [[H * 0.052, '#150e26'], [H * 0.030, '#d9d2e6']].forEach(([lw, col]) => {
                ...
                [1.5, 2.15, 2.8].forEach(k => {                                                   // 갈비 아치(위로 솟은 반원)
                    ctx.beginPath(); ctx.arc(sx + r * k, sy + r * 0.58, r * 0.52, Math.PI, Math.PI * 2); ctx.stroke();
                });
                ctx.lineCap = 'butt';
            });
            const skullP = () => { ctx.arc(sx, sy, r, 0, Math.PI * 2); };
            const jawP = () => { ctx.rect(sx - r * 0.5, sy + r * 0.55, r * 1.15, r * 0.60); };
            ctx.strokeStyle = '#150e26'; ctx.lineWidth = H * 0.024;
            [skullP, jawP].forEach(p => { ctx.beginPath(); p(); ctx.stroke(); });
            fill(ctx, '#d9d2e6', skullP);                                                        // 두개골
            fill(ctx, '#d9d2e6', jawP);                                                          // 턱
            fill(ctx, '#3a2a52', () => ctx.arc(sx - r * 0.34, sy - r * 0.08, r * 0.26, 0, Math.PI * 2));   // 눈구멍
            fill(ctx, '#3a2a52', () => ctx.arc(sx + r * 0.34, sy - r * 0.08, r * 0.26, 0, Math.PI * 2));
            [-0.28, 0, 0.28].forEach(dx =>                                                        // 이빨 골
                fill(ctx, '#3a2a52', () => ctx.rect(sx + r * (dx - 0.06), sy + r * 0.58, r * 0.12, r * 0.42)));
            const x = W * 0.365, y = H * 0.92, l = H * 0.10;                                      // 곁뼈 하나
            const boneP = [() => ctx.rect(x, y, l, H * 0.022), () => ctx.arc(x, y + H * 0.011, H * 0.022, 0, Math.PI * 2), () => ctx.arc(x + l, y + H * 0.011, H * 0.022, 0, Math.PI * 2)];
            ctx.strokeStyle = '#150e26'; ctx.lineWidth = H * 0.018;
            boneP.forEach(p => { ctx.beginPath(); p(); ctx.stroke(); });                          // 합집합 키라인
            boneP.forEach(p => fill(ctx, '#d9d2e6', p));
        };
        ...
        (() => {
            ...
            fill(ctx, 'rgba(255,255,255,.20)', () => ctx.rect(bx + bw * 0.12, by, bw * 0.13, bh + ey * 2)); // 세로 하이라이트
        ...
        /* 철망 마감(3차 재채점 교집합 ⓓ): 크로스해치가 프레임 없이 공중에서 끊겨 '깨진 무늬'로
           읽혔다 — 상·하단 가로 레일 + 포스트 2본 + 포스트 캡으로 '울타리 구조물'을 세운다.
           위치·범위(0~0.22W, y0.50~0.96H)는 사양 그대로. */
        ctx.save();
        ctx.strokeStyle = 'rgba(210,205,224,.88)'; ctx.lineWidth = H * 0.024;
        ctx.beginPath(); ctx.moveTo(0, H * 0.505); ctx.lineTo(W * 0.22, H * 0.505); ctx.stroke();  // 상단 레일
        ctx.beginPath(); ctx.moveTo(0, H * 0.935); ctx.lineTo(W * 0.22, H * 0.935); ctx.stroke();  // 하단 레일
        ctx.lineWidth = H * 0.028;
        [W * 0.022, W * 0.21].forEach(px => {                                                      // 포스트 2본
            ctx.beginPath(); ctx.moveTo(px, H * 0.50); ctx.lineTo(px, H * 0.945); ctx.stroke();
        });
        [W * 0.022, W * 0.21].forEach(px => {                                                      // 포스트 캡(둥근 머리 + 어두운 키라인)
            ctx.beginPath(); ctx.arc(px, H * 0.495, H * 0.021, 0, Math.PI * 2);
        ...
        [[0.045, 0.600, 0.52, 1, 0], [0.125, 0.578, 0.66, 0, 1], [0.225, 0.608, 0.48, 1, 1],
        [0.310, 0.588, 0.60, 0, 0], [0.360, 0.618, 0.44, 1, 1], [0.625, 0.585, 0.58, 1, 0]].forEach(([zx, zy, zs, arm, tone]) => {
        ...
        /* 팔꿈치 융합 패치(R2 — R1 A '직선 원통 3마디 + 볼조인트 = 레고 팔'): 관절 이음매의 검정
           ...
           실루엣에만 남는다). */
        on(ctx, circle(ctx, S, 1.406, 0.480, 0.046), '#4a8f47');                                  // 왼 팔꿈치
        on(ctx, circle(ctx, S, 1.948, 0.897, 0.043), '#3d7a3b');                                  // 오른 팔꿈치
        ...
        ink(ctx, S, poly(ctx, S, [[1.446, 0.630], [1.556, 0.612], [1.578, 0.706], [1.544, 0.686], [1.516, 0.728], [1.480, 0.698], [1.446, 0.724]]), '#41823f', 0.028);   // 왼 어깨
        ink(ctx, S, poly(ctx, S, [[1.830, 0.668], [1.936, 0.652], [1.956, 0.748], [1.922, 0.726], [1.894, 0.768], [1.858, 0.736], [1.830, 0.762]]), '#41823f', 0.028);   // 오른 어깨
        /* 목(R2 A '머리가 풍선처럼 떠 있다'): 머리보다 먼저 그려 머리 밑에 깔리는 짧은 목 기둥 —
           머리 기울기와 같은 쪽으로 살짝 물려 어깨선과 잇는다. */
        ink(ctx, S, rrect(ctx, S, 1.622, 0.545, 0.118, 0.085, 0.030), '#3a763a', 0.026);
        ...
        [[1.578, 0.372, 0.044, 0.028, -0.018], [1.638, 0.362, 0.062, 0.032, -0.008], [1.702, 0.364, 0.052, 0.030, 0.012], [1.756, 0.378, 0.038, 0.024, 0.022]].forEach(([hx, hy, hh, hw, lean]) =>
            ink(ctx, S, poly(ctx, S, [[hx - hw, hy], [hx + lean, hy - hh], [hx + hw, hy]]), '#3f7d3c', 0.022));
        ink(ctx, S, ell(ctx, S, 1.536, 0.470, 0.034, 0.046), '#4f9a4b', 0.026);                   // 귀(왼 실루엣 밖으로)
        ...
        on(ctx, rrect(ctx, S, 1.672, 0.372, 0.018, 0.200, 0.009), 'rgba(0,0,0,.34)');             // 뒤통수 스티치 흉터
        ...
        ink(ctx, S, bar(ctx, S, 1.545, 0.272, 1.310, 0.148, 0.058), '#e6e2d2', 0.024);            // 자루(주먹→좌상)
        ...
        ink(ctx, S, ell(ctx, S, 1.512, 0.290, 0.072, 0.064, -0.48), '#4a8f47', 0.028);            // 주먹(뼈 하단을 감아쥔 타원 한 덩어리)
        ctx.save();
        ctx.beginPath(); ell(ctx, S, 1.512, 0.290, 0.072, 0.064, -0.48)(); ctx.clip();
        [[1.532, 0.252], [1.546, 0.294]].forEach(([gx, gy]) =>
            on(ctx, rrect(ctx, S, gx - 0.012, gy - 0.052, 0.024, 0.104, 0.012), 'rgba(0,0,0,.26)'));   // 손가락 골 2줄(뼈 자루와 직교 방향)
        ctx.restore();
        on(ctx, ell(ctx, S, 1.492, 0.272, 0.026, 0.020), 'rgba(255,255,255,.20)');                // 주먹 광
    };
})(IconGen);
