    ...
    makeMountMesh(name, rarity) {
        ...
        if (formKey === 'flat') { // 호버형: 호버보드/호버 디스크 — 넓적한 발판 + 아래 추진 노즐
            ...
            if (whale) {
                ...
                for (const s of [-1, 1]) { const hn = cn(0.02, 0.09, s * 0.042, 0.58, 0.95, dark); hn.rotation.x = -0.7; } // 뿔
            ...
            sp(0.22, 0, CRAB ? 0.21 : 0.24, 0, mat, bs[0], bs[1], bs[2]);
            ...
            } else if (name === 'Pony') {                  // 조랑말: 목~등을 덮는 짧은 갈기 + 앞머리
            ...
            let eyeMeshes = crabEyes;
            if (!CRAB) {
                // 당나귀 머리는 **말보다 크고 길다** — 같은 두상 배율을 쓰면 귀만 긴 조랑말이 된다.
                // ⚠️ 두상 재질도 당나귀만 `mat`(몸색)으로 내린다. 공용값 `light` 는 에픽 초록에서
                //    거의 흰색으로 타는데, 거기에 밀리 무늬(회백)까지 얹으면 **머리 전체가 흰 덩어리**가
                //    돼 무늬가 정보 노릇을 못 한다(1판 실측). 무늬가 읽히려면 바탕이 몸색이어야 한다.
                // 거북 머리는 **작고 둥글다** — 말 두상(z 1.35 로 길쭉한 주둥이)을 그대로 쓰면
                // 등딱지를 아무리 잘 지어도 '껍질 진 말'이 된다.
                const hs = name === 'Donkey' ? [0.86, 0.86, 1.60]
                         : name === 'Turtle' ? [0.78, 0.80, 0.98] : [0.92, 0.88, 1.35];
                HEADPART(sp(0.13, 0, headY, headZ, name === 'Donkey' ? mat : light, hs[0], hs[1], hs[2])); // 머리
            ...
            if (CRAB) crabLegs();
                ...
                const lw = name === 'Turtle' ? 1.55 : 1;
                const leg = cy(0.05 * lw, 0.045 * lw, 0.24, 0, 0.1 - HIP_Y, 0, dark);
