...
### ⚠️ 장비에는 등급 축이 없다 (사용자 확정 2026-08-18, 클론 구현 정정)

원본도 장비는 **시대 티어 10단계**로만 구분한다(위 '등급 체계' 절) — Common/Rare/… 6등급은 펫·알·탈것·스킬 축이다.
그런데 클론은 2026-08-18까지 장비에도 **화면에 안 보이는 등급을 따로 굴려** 아래 셋을 흔들고 있었다:
주스탯 배율 `RARITY_MULT`(1~6.5) · 서브스탯 개수 상한 · 판매가 배수. 전부 제거했다.

| 항목 | 이전(클론) | 지금 |
|---|---|---|
| 주스탯 | `티어기본 × 1.01^(lv−1) × RARITY_MULT(1~6.5)` | `티어기본 × 1.01^(lv−1)` |
| 서브스탯 개수 | 1 ~ (등급순번+1), 최대 4 | 1 ~ 4 균등 |
| 판매가 | `20 × 1.01^(lv−1) × RARITY_MULT × (1+보너스)` | `20 × 1.01^(lv−1) × (1+보너스)` = **원본 공식** |
| 장식(문장·젬·오브·발광) | 등급색·등급 문턱 | 시대 장식색(`AGE_ACCENT`)·시대 문턱(`gearOrnateTier`) |

**전력 영향**: 제거된 등급 배수의 기댓값은 대장간 레벨에 따라 1.34배(Lv1) ~ 1.83배(Lv60)였다.
즉 같은 시대·레벨에서 장비 주스탯이 **평균 0.55~0.75배로 내려간다.** 다만 시대 한 단계가 ×6이므로
없어진 폭 전체가 **1시대(×6)보다 작다** — 진행 곡선상 약 0.3시대 분량의 하향이고, 대신 같은 장비가
이유 없이 6.5배 차이 나던 변동이 사라진다.

**펫·탈것과의 정합**: 펫·탈것 기준축(`Forge.gearSumAtkAt/gearSumHpAt`)은 **원래부터 등급 배수 없이**
`티어기본 × 레벨배율`로 계산됐다. 즉 지금까지는 실제 장비만 기준축보다 1.3~1.8배 위에 떠 있었고,
이번 제거로 **장비가 자기 기준축과 정확히 일치**한다(펫·탈것이 상대적으로 약하던 어긋남이 해소).
...
- **웨이브 수 1~3 (사용자 지시 2026-08-18)** — 입장 시 1~3 중 하나로 굳히고 마지막 웨이브가 보스.
  메인 스테이지는 5웨이브 그대로. (`Dungeons.rollWaves` / `Combat.totalWaves`)
- **클리어 보상 = 100 + 2×(단계−1) 선형 (사용자 지시 2026-08-18)** — 던전별 지수 스케일 폐기,
  보상 '종류'는 그대로 두고 수량만 통일. 기술트리 배율 노드는 이 값 위에 곱해진다.
  Hammer Thief는 위 표대로 **해머/코인 2종만**(붙어 있던 태엽 ⚙️ 부가 보상 제거 — 태엽은 리그·진행 패스에서 수급)
